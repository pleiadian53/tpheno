...
def loadSparseTSet(**kargs): 
    ...
    X, y = tsHandler.loadSparse(index=kargs.get('index', 0), meta=kargs.get('meta', None)) 
...
def multiClassEvaluateSparse(**kargs):
    ...
    def roc_cv(X, y, classifier, fpath=None, target_labels=[]):    
        ...
        # resturn max performance and min performance; key: 'min': label -> auc, 'max': label -> auc
    ...
    return res # return best and worst performance in terms of labels and their corresponding AUC (used for multiple trials)
...
def test(**kargs): 
    ...
    cohort = 'CKD'
    policy_segment = 'regular'  # policy values: {'regular'/noop, 'two', 'prior', 'posterior', 'complete', }
    ...
    X, y = t_model(min_ncodes=10, 
                   ...
                   load_model=kargs.get('load_model', False), test_model=False, 
                   include_augmented=False)  # use the default d2v method defined in vector module
    ...
    if tsHandler.is_sparse(): # check tsHandler.d2v
        t_classify(X=X, y=y, mode='multiclass', n_per_class=nPerClass, tset_dtype='sparse', drop_ctrl=drop_ctrl,
            clf_name='gradientboost', n_estimators=nTrees) 
        # t_classify(mode='multiclass', n_per_class=nPerClass, tset_dtype='sparse') 
    else: 
        # load training set from the saved file
        t_classify(mode='multiclass', n_per_class=None, tset_dtype='dense', drop_ctrl=drop_ctrl, 
            clf_name='gradientboost', n_estimators=nTrees) # don't use 'classifier_name'
