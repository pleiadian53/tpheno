...
# this returns the distance matrix with Gower's distance (a dissimilarity matrix)
g.dist = daisy(data, metric="gower", type=list(symm=2))

# write CSV 
D = as.matrix(g.dist)
# write.csv(D, file = "dissimilarity.csv", row.names = FALSE)
write.table(D, file = "dissimilarity.csv",row.names=FALSE, na="",col.names=FALSE, sep=",")
