...
def find_optimal_k(X, y=None, **kargs): 
    ...
    max_n_clusters = kargs.get('max_n_clusters', 20)
    nrefs = kargs.get('nrefs', 15)

    k, gapdf = optimalK(X_proj, nrefs=nrefs, maxClusters=max_n_clusters, step=step)
...
def t_gap_sprint2(**kargs): 
    ...
    bestK = find_optimal_k(X, y=y, reduce_dimension=True, identifier='sprint_Rtsne', 
        step=step, max_n_clusters=max_n_clusters)
    print('result> best k with dim reduction (via t-SNE): %d' % bestK)
    bestK = find_optimal_k(X, y=y, reduce_dimension=False, identifier='sprint_Rnoop', 
        step=step, max_n_clusters=max_n_clusters)
    print('result> best k WITHOUT dim reduction: %d' % bestK)
