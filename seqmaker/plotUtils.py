...
def makeBarPlot(x, y, title_x=None, title_y=None, color_marker='#C8E3EF', **kargs):
    """
    ...
    """
    def make_titles(params):  # kargs: cohort
        # ad-hoc plug-in, cohort-dependent titles, perhaps turn this into some class method? 
        cohort = kargs.get('cohort', 'CKD')
        default_title_x, default_title_y = "Class labels", "Values"
        params['title_y'] = title_y
        if cohort.startswith("gen") or not cohort: # generic  
            params['title_x'] = default_title_x if title_x is None else title_x
            params['title_y'] = default_title_y if title_y is None else title_y
        elif cohort == 'CKD': 
            params['title_x'] = "CKD stages" if title_x is None else title_x
            params['title_y'] = "Area under the curve" if title_y is None else title_y
        elif cohort == 'PTSD':
            params['title_x'] = "PTSD subtypes" if title_x is None else title_x
            params['title_y'] = "Area under the curve" if title_y is None else title_y
        else: 
            raise ValueError, "Unknown cohort: %s" % cohort
        return 
    def get_prefix(): 
        prefix = kargs.get('outputdir', None)
        if prefix is not None and os.path.exists(prefix): return prefix 
        cohort_name = kargs.get('cohort', 'CKD')
        return Graphic.getPath(cohort=cohort_name, dir_type='plot', create_dir=True)
    def get_fname(): 
        fname = kargs.get('outputfile', None)
        if fname: return fname 
        cohort_name = kargs.get('cohort', 'CKD')
        return Graphic.getName(cohort=cohort_name, prefix='auc_per_class', identifier='barplot', ext_plot='tif')
    def customize_individual_bar_color(): 
        pass
    ...
    # configure graph trace
    trace = {}
    trace['x'] = x 
    trace['y'] = y 
    trace['color_marker'] = color_marker  # '#0066ff'
    params['traces'].append(trace)
    
    # titles 
    make_titles(params) # kargs: cohort

    # I/O path 
    fname = get_fname()
    basedir = get_prefix() # kargs: prefix, cohort  # check in that order
    params['opath'] = os.path.join(basedir, fname)

   
    # [plug-in] this will call plot.ly interface 
    t_plotly(params)
...
def t_heatmap_plotly(X, y, **kargs): 
    # import plotly.plotly as py
    # import plotly.graph_objs as go    
    basedir = os.path.join(os.getcwd(), 'test')
    print('input> dim | X: %s, y: %s | d2v: %s, seq: %s' % \
            (str(X.shape), str(y.shape), kargs.get('d2v_method', '?'), kargs.get('seq_ptype', '?') ))

    labels = y 
    features = kargs.get('features', ['f_%s' % i for i in range(X.shape[1])] )
    trace = go.Heatmap(x=features, y=labels, z=X)
    ...
    # save file
    kargs['graph_ext'] = 'png'  # plotly only supports: png, svg, jpeg, and pdf
    fpath = os.path.join(basedir, seqparams.name_image_file(descriptor='heatmap', **kargs))
    base, fname = os.path.dirname(fpath), os.path.basename(fpath)
    print('output> saving heatmap at %s' % fpath)
