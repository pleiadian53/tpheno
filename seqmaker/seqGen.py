...
#########################################################################
#
#  New version of seqMaker series
#  
#  Similar to {seqMaker, seqMaker2, seqMaker3} creates documents consisting of 
...
def make_seq(**kargs): 
    ...
    def codify_seq(dfv): # data from the same visit (the same date)
        ...
        n_skipped = 0
        nrow = dfv.shape[0]
        if remove_source_dups: # remove duplicate entries? True by default 
            seen = set()
            # for i, e in enumerate(dfv[col].values): 
            n_unmapped = 0 
    ...
    for e in df_condition['condition_source_value'].values:  # or use 'condition_concept_id'
        ...
        if len(el) > 1: 
            ...
            if pmed.isICD(e):  # extra dot S53. 920. is considered valid 
                ...
                ep = e  # don't try to convert it (too complicated, just treat it as noise)
                if pmed.isICD(ep): 
                    noncoded_condition[e] = ep 
                    is_resolved = True
                else: 
    ...
    ### use cohort.makeLabeledSeq to create labeled documents  ... 03.18
    ...
    for e in df_condition['condition_source_value'].values:  # or use 'condition_concept_id'
        ...
        if len(el) > 1: 
            ...
            #         # e.g. 999999 => 999999
                ...
                # 1. leave it as it is
                # 2. map it
            pass  # take whatever form it is  ... 08.2018
