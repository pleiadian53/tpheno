...
### System Variables ### 
# DBScope = 'ohdsi.west'
DBScope = 'ohdsi_west_pending_20161027.dbo'
...
def searchByID(**kargs): 
    """
    Search DB given a set of person IDs and generate the intermediate dataframe
    ...
    Use
    ---
    1. Given the curated (annotated) data (e.g. CdwCkdCohort_20170817.csv)
       Extract person_id and use it as an input to this function

    """
    def save(df, fsep='|', overwrite_=True): # <- table, identifier
        outputdir = kargs.get('output_dir', sys_config.read('DataExpRoot')) 
        fp = os.path.join(outputdir, '%s-query_ids-%s.csv' % (table, identifier))
        if not os.path.exists(fp) or (os.path.exists(fp) and overwrite_): 
            print('output> writing querying result to %s' % fp)
            df.to_csv(fp, sep=fsep, index=False, header=True, encoding='utf-8')
    ...
    if save_intermediate: 
        # e.g. condition_occurrence-query_ids-src.csv, drug_exposure-query_ids-src.csv, condition_occurrence-query_ids-augment.csv
        save(df, fsep='|', overwrite_=overwrite_intermediate) # <- table, identifier
...
def searchByDiag(codes, **kargs): 
    """
	Given a set of diagnostic codes, find corresponding cohort (from condition table)
...
def filterCandidatesByCodes(codes, dataframe=None, relation='and', **kargs): 
    ...
    """
    def get_prefix_dir():  # <- cohort
        # e.g. cohort=CKD 
        #      => tpheno/data-exp/CKD
        cohort_name = kargs.get('cohort', None)
        prefix_dir = sys_config.read('DataExpRoot')
        if cohort_name: 
            fpath = os.path.join(prefix_dir, cohort_name)
            assert os.path.exists(fpath), "Invalid cohort directory (cohort=%s): %s" % (cohort_name, fpath)
            return seqparams.getCohortGlobalDir(cohort, basedir=fpath, create_dir=False)
        return prefix_dir
    def get_input():  # <- ifile, prefix_dir, {field_source, field_date}, fsep
        dfi = dataframe
        if dfi is None:  # assign default input dataframe
            # basedir = get_prefix_dir() 
            ifile = kargs.get('ifile', '%s-query_codes.csv' % table)
            ipath = os.path.join(get_prefix_dir(), ifile)
            print('info> loading default precomputed cohort dataframe at %s' % ipath)
            dtypes = {field_source: str}
            dfi = pd.read_csv(ipath, sep=fsep, index_col=False, header=0, error_bad_lines=True, 
                                     parse_dates=[field_date], dtype=dtypes)
        else: 
            print('info> loading user provided input dataframe (dim=%s)' % str(dataframe.shape))
        assert dfi is not None and not dfi.empty, "No condition data found!"
        return dfi
    def test_preconditions(): 
        assert hasattr(codes, '__iter__'), "Input codes are ill-formatted: %s" % codes
        return
    def test_match(m=100): 
        if n_candidates % m == 0: 
            print('test_match> Found %d candidates (out of %d) | op=%s, common codes:\n  + %s\n' % (n_candidates, cnt, relation, commonCodes))
        return
    def summary_stats():
        n_unique = len(set(df_condition[field_id].values))
        print('filterCandidatesByCodes> df_condition > unique IDs: %d, dim: %s' % (n_unique, str(df_condition.shape))) 
        ...
        print('  + most representative code: %s' % repr_code)

        return
    def save_candidates(): # <- targetCodes, basedir, field_id='person_id'
        fpath = os.path.join(get_prefix_dir(), 'candidates-%s-%s.csv' % (targetCodes[0], len(targetCodes)))
        ...
        return 
    def show_params(): 
        print('filterCandidatesByCodes> Parameters')
        print('  + basedir: %s' % basedir)
        print('  + save_intermediate? %s' % save_intermediate)
        print('  + simplify code? %s' % base_only)

    import collections

    # [params]
    basedir = get_prefix_dir()  # this directory can serve as input as well as output
    # table = kargs.get('table', tb_condition)
    save_intermediate = kargs.get('save_intermediate', True)
    # exclude_ids = set(targets.target_set)
    base_only = kargs.get('simplify_code', False) # pmed.containsBaseForm(codes)
    fsep = '|'
    ...
    if base_only: 
        ...
        for pid, df in df_condition.groupby('person_id'): 
            ...
            if relation == 'and': 
                ...
                if codeSet.issubset(refcodes_base): 
                    candidates[pid] = refcodes
                    n_candidates = len(candidates)
                    if n_candidates % 50 == 0: 
                        print('filterCandidatesByCodes> Found %d candidates (out of %d) | op=%s, common: %s' % \
                            (n_candidates, cnt, relation, commonCodes))
                ...
                if len(commonCodes) >= minNIntersect: 
                    candidates[pid] = refcodes 
                    n_candidates = len(candidates)
                    test_match(m=100)
                else: 
            ...
            if n_nomatch > 0 and n_nomatch_cycle % 10 == 0: 
        ...
        for pid, df in df_condition.groupby('person_id'): 
            ...
            if relation == 'and': 
                ...
                if len(commonCodes) >= minNIntersect:  # size of intersection is at least minN...
                    candidates[pid] = refcodes 
                    n_candidates = len(candidates)
                    test_match(m=100) # n_candidates, commonCodes, relation, cnt 
                else: 
                    n_nomatch += 1
...
def t_search_ckd(**kargs): 
    """
    Given the labeled data, make intermediate files asociated with the cohort (CKD)

    Workflow
    --------
    a. t_search_<disease> => makeMCS()
    """
    def load_source(fname, basedir=None, cohort='CKD', sep=','): # <- fname, header, basedir
        # load annotated data, in which the labels (CKD stages) are determined by eMerge algorithms
        # e.g. eMerge_NKF_Stage_20170818.csv    ['patientId', 'Case_Control_Unknown_Status', 'NKF_Stage', ]
        #      CdwCkdCohort_20170817.csv     ['person_id', 'NKF_Stage', 'CaseControlUnknownStatus']
        if basedir is None: os.path.join(sys_config.read('DataExpRoot'), cohort)
        assert os.path.exists(basedir), "Invalid prefix directory: %s" % basedir
        fpath = os.path.join(basedir, fname)
        assert os.path.exists(fpath), "Invalid source directory: %s" % fpath
        df = pd.read_csv(fpath, sep=sep, header=0, index_col=False, error_bad_lines=True)
        return df
    def filter_by_attributes(df, inclusion=None): # <- df
        inclusion = ['Case', 'Control']  # 'Unknown'
        N0 = df.shape[0]
        cond = df['CaseControlUnknownStatus'].isin(inclusion)
        df = df.loc[cond]
        N1 = df.shape[0]
        print('  + filter nrow: %d -> %d' % (N0, N1))
        return df
    def grouper(n, iterable, padvalue=None):
        # also see chunk()
        # grouper(3, 'abcdefg', 'x') --> ('a','b','c'), ('d','e','f'), ('g','x','x') 
        return izip(*[chain(iterable, repeat(padvalue, n-1))]*n)
    def save(df, table='generic', identifier=None, fsep='|', overwrite_=True): # <- cohort_name
        outputdir = basedir 
        if identifier is None: identifier = cohort_name
        fp = os.path.join(outputdir, '%s-query_ids-%s.csv' % (table, identifier))
        if not os.path.exists(fp) or (os.path.exists(fp) and overwrite_): 
            print('save> writing querying result to %s' % fp)
            df.to_csv(fp, sep=fsep, index=False, header=True, encoding='utf-8')
    ...
    # [input] set B 
    fname = kargs.get('annotated_file', 'CdwCkdCohort_20170817.csv')    # size of cohort: 672858 (0.6M) unique persons
    header = ['person_id', 'NKF_Stage', 'CaseControlUnknownStatus']
    fID = header[0]

    df = load_source(fname=fname, basedir=basedir)
    filter_by_attributes(df) # keep only 'Case' and 'Control'

    print('io> loading patient list (cohort=%s) of dim: %s' % (cohort_name, str(df.shape)))
    print df.head(10)
    print df.tail(10)

    person_idx = list(df[fID].unique())
    nPersons = len(person_idx)
    print('t_search_ckd> size of cohort: %d unique persons, %d annotated instances' % (nPersons, df.shape[0]))

    nMaxCohortSizePerUnit = 5000
    tLargeCohort = True if nPersons > nMaxCohortSizePerUnit else False
    tSaveCombinedDF = tLargeCohort
    ...
    for tb in tables:  
        ...
        # params: identifier (/cohort)
        if tLargeCohort: 
            n_subset = nMaxCohortSizePerUnit
            n_persons = 0  
            dfx = []
            try: 
                for i, person_subset in enumerate(chunks(person_idx, n_subset)):   # 
                    fild_id = '%s-%s' % (cohort_name, i)
                    df = searchByID(table=tb, ids=person_subset, cohort=cohort_name, save_intermediate=False, identifier=fild_id)
                    dfx.append(df)  # if the memory capacity is large enough
                    n_persons += len(person_subset)
                    print('   + %s dataframe (part #%d) > dim: %s | n_persons_acc: %d' % (tb, i, str(df.shape), n_persons))
            except Exception, e: 
                print('Warning: DB Error (%s)\nSave whatever has been collected now (n_persons: %d)\n' % (e, n_persons))

            if tSaveCombinedDF: 
                df = pd.concat(dfx, ignore_index=True)
                save(df, table=tb, identifier=cohort_name, fsep='|', overwrite_=True) # <- cohort_name
            else: 
                dfx = None; gc.collect()
        else: 
...
def readAnnotatedDataSource(fname, **kargs):
    """

    Memo
    ----
    1. Example annotated set: 
         'CdwCkdCohort_20170817.csv' 
              ['person_id', 'NKF_Stage', 'CaseControlUnknownStatus']

          'eMerge_NKF_Stage_20170818.csv'
              ['patientId', 'Case_Control_Unknown_Status', 'NKF_Stage', ] 

    2. 'CdwCkdCohort_20170817.csv':
            size of cohort: 672858 (0.6M) unique persons
    """
    def load_source(sep=','): # <- fname, header, basedir
        # load annotated data, in which the labels (CKD stages) are determined by eMerge algorithms
        # e.g. eMerge_NKF_Stage_20170818.csv    ['patientId', 'Case_Control_Unknown_Status', 'NKF_Stage', ]
        #      CdwCkdCohort_20170817.csv     ['person_id', 'NKF_Stage', 'CaseControlUnknownStatus']
        assert os.path.exists(basedir), "Invalid prefix directory: %s" % basedir
        ...
        df = pd.read_csv(fpath, sep=sep, header=0, index_col=False, error_bad_lines=True)
        return df
    def filter_by_attributes(df, col, inclusion=None): # <- df
        if inclusion is None: inclusion = ['Case', 'Control']  # 'Unknown'
        N0 = df.shape[0]
        cond = df[col].isin(inclusion)
        df = df.loc[cond]
        N1 = df.shape[0]
        print('  + filter nrow: %d -> %d' % (N0, N1))
        return df
    ...
    ctrl_cohort_name = '%s-Negative' % cohort_name
    tConceptGrouping = True
    id_field, target_field, case_control_field = kargs.get('id_field', 'person_id'), kargs.get('target_field', 'NKF_Stage'), \
        kargs.get('case_control_field', 'CaseControlUnknownStatus')
    ...
    fID = header[0]
    filter_by_attributes(df, col=case_control_field) # keep only 'Case' and 'Control'
...
def t_getLabels(**kargs):
    ...
    cohort_name = kargs.get('cohort', 'CKD')
    df_src = readAnnotatedDataSource(cohort=cohort_name, fname=srcFiles[cohort_name])
...
def loadConditionDF(identifier, inputdir=None, prefix='condition_occurrence', delimit='|'): 
    ...
    1. example intermediate files
       ...
       data-exp/sequencing/condition_occurrence-{0-9}.csv
          => identifier <- 0, 1, ... 9
    """
    def resolve_input(): # <- inputdir
        prefix_dir = inputdir
        if prefix_dir is None: 
            basedir = sys_config.read('DataExpRoot')  # tpheno/data-exp
            prefix_dir = os.path.join(basedir, 'sequencing')  # tpheno/data-exp/sequencing
        assert os.path.exists(prefix_dir), "Invalid input dir:\n%s\n" % prefix_dir

        ifile = '%s-%s.csv' % (prefix, identifier)
        fpath = os.path.join(prefix_dir, ifile)
        assert os.path.exists(fpath), "Invalid intermediate file: %s" % ifile 
        return fpath
    def verify(df, col_date='condition_start_date', n_latest=5): 
        assert df is not None and not df.empty, "Null dataframe."
        nrow, ncol = df.shape[0], df.shape[1]
        print('  + intermediate file | prefix: %s, dim: %d by %d' % (prefix, nrow, ncol))

        print('  + show %d latest timestamps' % n_latest)
        for i, t in enumerate(df.sort_values([col_date, ], ascending=False, inplace=False)[col_date][:n_latest]): 
            print("... [%d] timestamp: %s" % (i+1, t))
...
def loadDrugDF(identifier, inputdir=None, prefix='drug_exposure', delimit='|'): 
    ...
    def resolve_input(): 
        prefix_dir = inputdir
        if inputdir is None: # assign default
            basedir = sys_config.read('DataExpRoot')  # tpheno/data-exp
            prefix_dir = os.path.join(basedir, 'sequencing')  # tpheno/data-exp/sequencing
        assert os.path.exists(prefix_dir), "Invalid input dir:\n%s\n" % prefix_dir
        ...
        assert os.path.exists(fpath), "Invalid intermediate file: %s" % ifile 
        return fpath
    def verify(df, col_date='drug_exposure_start_date', n_latest=5): 
        assert df is not None and not df.empty, "Null dataframe."
        nrow, ncol = df.shape[0], df.shape[1]
        print('  + intermediate file | prefix:%s, dim: %d by %d' % (prefix, nrow, ncol))
...
def t_augment_cohort(**kargs):
    ...
    def select_by_diag(df, codes, rel='or', id_='?'):  # <- diagCodes
        ...
        candidates = filterCandidatesByCodes(codes=codes, dataframe=df, relation=rel, simplify_code=False)  # person_id -> {code}
        uids = set(candidates.keys())  # candidates: person_id -> {code}
        print('  + found %d unique candidates in group %s' % (len(uids), id_))
    ...
    def save_intermediate(df, prefix, suffix='query_ids-augment', delimit='|'): # df_condition, df_drug
        ...
        if not df.empty:
            ...
            # if not os.path.exists(fp) or (os.path.exists(fp) and overwrite_intermediate): 
            print('  + writing augmented candidates (cohort=%s) to %s' % (cohort_name, fp))
    ...
    def summary_stats(): # <- all candidates 
        print('  + n_unique: %d' % len(allCandidates))  # 
        return 
        
    import seqMaker2 as smaker
    import seqparams
    from cohort_selection import ccs
    ...
    for i in range(0, 10): # condition_occurrence-0.csv, drug_exposure-0.csv 
        df_condition = loadConditionDF(identifier=i, inputdir=basedir_sequencing) 
        df_drug = loadDrugDF(identifier=i, inputdir=basedir_sequencing)
        idx = select_by_diag(df_condition, codes=diagCodes, id_=i)

        # exclusion list (don't include those already labeled)
        if tExcludeLabeled: 
            idx0 = set(idx)
            idx = set(idx0) - set(labeledPersonIDs)
            print('t_augment_cohort> Group %s > after excluding labeled cadidates (n=%d) => n(candiates)=%d -> %d remaining' % \
                (i, len(labeledPersonIDs), len(idx0), len(idx)))

        # groupToIDs[i] = idx
        allCandidates.update(idx)
        if len(idx) > 0: 
            dfci = select_by_ids(df_condition, person_ids=idx)
            assert dfci is not None and not dfci.empty
            dfc.append(dfci)
    ...
    createDocFromDF(cohort=cohort_name,
        condition_table=df_condition,   # 'condition_occurrence-query_ids-augment.csv'
        drug_table=df_drug,  # 'drug_exposure-query_ids-augment.csv'
        meta='augmented')  
        
    return 

def makeMCS(cohort, **kargs):
    """

    Memo
    ----
    1. see also t_augment_ckd()
    ...
    def load_labeled_candidates(prefix='condition_occurrence', suffix='query_ids-src', col='person_id'): 
        ...
        print('  + found %d unique IDs from the labeled candidates' % len(uids))
        return uids
    def save_intermediate(df, prefix, suffix='query_ids', delimit='|'): # df_condition, df_drug
        # save new intermediate files that include the new candiates
    ...
    cohort_name = cohort # kargs.get('cohort', 'CKD')
    basedir = get_global_cohort_dir()
    identifier = kargs.get('identifier', 'query_ids-%s' % cohort_name)

    ### load intermediate files 
    # e.g. tpheno/data-exp/CKD/condition_occurrence-query_ids-CKD.csv
    #      header condition_source_value|condition_start_date|drug_exposure_start_date|drug_source_value|person_id
    df_cond = loadConditionDF(identifier, prefix='condition_occurrence', inputdir=basedir, delimit='|') 
    # e.g. tpheno/data-exp/CKD/drug_exposure-query_ids-CKD.csv
    df_drug = loadDrugDF(identifier, prefix='drug_exposure', inputdir=basedir, delimit='|')

    ### seqMaker2.make_seq2 expects the following headers
    #     condition: ['person_id', 'condition_start_date', 'condition_source_value', ]
    #     medication: ['person_id', 'drug_exposure_start_date', 'drug_source_value', ]

    createDocFromDF(cohort=cohort_name,
        condition_table=df_cond,   # 'condition_occurrence-query_ids-augment.csv'
        drug_table=df_drug)  # 'drug_exposure-query_ids-augment.csv'

    return

def summarizeCohort(cohort='CKD', inputdir=None, ifiles=[], seq_ptype='regular', **kargs): 
    """
    
    Memo
    ----
    1. number of patients (n_persons) 
        + n_persons  | >= 10 records
        + identifiability 
    2. sequence 
        + average length

    3. example source paths 
       inputdir = '<prefix>/tpheno/data-exp/CKD0'
       ifiles = ['condition_drug_timed_seq-CKD.csv', ]

    4. source reader examples: 
         sr.readDocFromCSV(cohort=cohort, inputdir=inputdir, ifiles=ifiles, complete=True) 

    5. parsing date/time  
       from dateutil import parser
       parser.parse("Aug 28 1999 12:00AM")

    6. CKD diagnosis codes
       585 585.1 585.2 585.3 585.4 585.5 585.6 585.9 792.5 V42.0 V45.1 V45.11 V45.12 V56.0 V56.1 V56.2 V56.31 V56.32 V56.8
          + use 585 as a proxy for first diagnosis

    """
    def get_global_cohort_dir(): 
        return seqparams.getCohortGlobalDir(cohort) # sys_config.read('DataExpRoot')/<cohort> 
    def get_size(D_in, min_size=1): # <- D 
        return sum([1 for d in D_in if len(d) >= min_size])
    def describe(alist): 
        # summary statistics (e.g. 5 number summary)
        return pd.Series(alist).describe()
    def test_mcs_integrity(D_in, T_in): 
        assert len(D_in) == len(T_in), "Dimension inconsistent size(d):%d != size(t):%d" % (len(D_in), len(T_in))
        for i, d in enumerate(D_in): 
            assert len(d) == len(T_in[i]), "Size inconsistent size(d):%d != size(t):%d" % (len(d), len(T_in[i]))
        return 
    def analyze_doc(D_in, min_size=1): 
        print("> size (complete): %d | size (n>=%d): %d" % (len(D_in), min_size, get_size(D_in, min_size)))

        ### document statistics 
        lengths = [len(d) for d in D]
        print("> mean(length):%f, std(length):%f" % (np.mean(lengths), np.std(lengths)))
        print("    + five number summary:\n%s\n" % describe(lengths))

        return
    def analyze_time(T_in, min_size=1, ignore_empty=False): 
        t_pairs, deltax = [], [] # datetime list
        Tu = [] # unique timestamps ~ number of visits
        # Td = []
        if ignore_empty: 
            for i, tx in enumerate(T_in):
                if len(tx) < min_size: continue
                t_min = datetime.strptime(tx[0],  '%Y-%m-%d')  # string parse time 
                t_max = datetime.strptime(tx[-1], '%Y-%m-%d')
                t_pairs.append((t_min, t_max))
            
                delta = (t_max-t_min).days
                # delta = relativedelta(t_max, t_min)  # can convert to n years m months d days
                deltax.append(delta)
                Tu.append(set(tx))
            assert len(Tu) <= len(T_in)
        else: 
            for i, tx in enumerate(T_in):
                if len(tx) >= min_size: 
                    t_min = datetime.strptime(tx[0],  '%Y-%m-%d')  # string parse time 
                    ...
                    delta = 0   # no records, assumed 0 days
                deltax.append(delta)
                Tu.append(set(tx))
            assert len(Tu) == len(T_in)

        # average time of medical history 
        nDays = sum(deltax)/len(Tu)
        nYears = nDays/365.25
        print('  + average length of medical records: %f (days) ~ %f (years)' % (nDays, nYears))

        # distinct clinical visits
        n_visits = [len(tu) for tu in Tu] # tu: a set of unique timestamps
        print("> mean(n_visits):%f, std(n_visits):%f" % (np.mean(n_visits), np.std(n_visits)))
        print("    + five number summary (# of unique visits):\n%s\n" % describe(n_visits))

        # global statistics
        tMin, tMax = min(t_pairs[0]), max(t_pairs[0])
        for i, tp in enumerate(t_pairs): 
            # t_min, t_max = min(tp), max(tp)
            t_min, t_max = tp
            if t_min < tMin: 
                tMin = t_min
            if t_max > tMax: 
                tMax = t_max
        print("  + Cohort %s has records dating from %s to %s" % \
            (cohort, datetime.strftime(tMin, '%Y-%m-%d'), datetime.strftime(tMax, '%Y-%m-%d')))

        return

    import pandas as pd
    import docProc, seqparams
    ...
    # if ifiles are specified, cohort is ignored
    #    kargs: predicate, source_type
    D, L, T = docProc.processDocuments(cohort=cohort, inputdir=inputdir, ifiles=ifiles, seq_ptype=seq_ptype, 
                                doc_filter_policy='minimal_evidence', min_ncodes=1)  # 'source_type', 'slice_policy'
    test_mcs_integrity(D, T) # D, L, T

    # size of data set 
    minSize = 10
    analyze_doc(D, min_size=minSize)

    # sequence statistics 
    # a. average length, std(length)
    lengths = [len(d) for d in D]
    print("> mean(length):%f, std(length):%f" % (np.mean(lengths), np.std(lengths)))
    print("  + five number summary:\n%s\n" % describe(lengths))
    ...
    #   format: 1991-02-21
    # a. oldest to latest per patient 
    # b. averaged clinical lengths 
    # c. averaged time-to-first diagnosis
    # d. average time-to-first effective diagnosis 
    print('summarizeCohort> analyzing original timestamps: T ...')
    analyze_time(T)

    print("summarizeCohort> analyzing timestamps (pre-diagnosis sequence): T2 ...")
    isCaseRoutine = ckd.isCase
    D2, T2 = docProc.cutByTimeToDiagnosis(D, T, is_case=isCaseRoutine, min_mention=1, drop_empty=False)
    test_mcs_integrity(D2, T2) # D, L, T
    analyze_time(T2); analyze_time(T2, ignore_empty=True)

    return

def makeLabeledSeq(**kargs):
    return makeMCSFromTimedSeq(**kargs)
def makeMCSFromTimedSeq(**kargs):
    def get_global_cohort_dir(): 
        return seqparams.getCohortGlobalDir(cohort_name) # basedir: sys_config.read('DataExpRoot')
    def fill_labels_via_lookup(dfsrc, dfmcs, col_id='person_id', col_label='NKF_Stage'): 
        lookup = dict(zip(dfsrc[col_id].values, dfsrc[col_label].values))  # id -> label 
        
        labels = []
        for i, (r, row) in enumerate(dfmcs.iterrows()): 
            labels.append(lookup[row[col_id]])
        
        dfmcs['label'] = labels 
        print('  + dfmcs header after fill: %s' % str(dfmcs.columns.values))
        return dfmcs
    def is_id_consistent(dfsrc, dfmcs, col_id='person_id'):
        idx1, idx2 = dfmcs[col_id].values, dfsrc[col_id].values 
        return len(idx1) == len(idx2) and len(set(idx1)) == len(set(idx2))
    def is_subset(dfmcs, dfsrc, col_id='person_id'): # MCS IDs must be a subset of (if not identical) the source IDs 
        return set(dfmcs[col_id]).issubset(dfsrc[col_id].values)
    def get_person_id_col(df): 
        pass
    ...
    # 
    header_src = ['person_id', 'NKF_Stage', 'CaseControlUnknownStatus']  # header for CdwCkdCohort_20170817.csv

    # use batchpheno.icd9utils.preproc_code() to process CCS ICD9 string
    cohort_name = kargs.get('cohort', 'CKD')
    outputdir = get_global_cohort_dir()

    ### input2: annotated source
    id_field, target_field= 'person_id', 'NKF_Stage' 
    fname = kargs.get('annotated_file',  'CdwCkdCohort_20170817.csv')
    dfsrc = readAnnotatedDataSource(fname, cohort=cohort_name, target_field=target_field)
    nP0 = dfsrc.shape[0]

    ### input: timed_seq
    stem, doctype = 'condition_drug', TDoc.doctypeToStem(doctype='timed')
    fname = '%s_%s-%s.csv' % (stem, doctype, cohort_name)
    fpath = os.path.join(outputdir, fname)
    assert os.path.exists(fpath), "Invalid input path to timed_seq (cohort=%s): %s" % (cohort_name, fpath)
    dfmcs = pd.read_csv(fpath, sep='|', header=0, index_col=False, error_bad_lines=True)

    tIDConsistent = True if is_id_consistent(dfmcs, dfsrc) else False 

    nP = dfmcs.shape[0]
    print('makeMCSFromSource> n_anno: %d =?= n_sequenced: %d > a subset of patients do not have data' % (nP0, nP))

    # deidentify 
    # dfmcs = dfmcs[['sequence', 'timestamp']]  # but not person_id

    # fill the labels
    if tIDConsistent: 
        fill_labels(dfsrc, dfmcs)
    else: 
        print('makeLabeledSeq> source and MCS are not ID-consistent.')
        assert is_subset(dfmcs, dfsrc, col_id='person_id'), "MCS IDs are not a subset of source IDs???"
        fill_labels_via_lookup(dfsrc, dfmcs, col_id=id_field, col_label=target_field)

    # test_mcs_integrity(dfmcs)

    stem, doctype = 'condition_drug', TDoc.doctypeToStem(doctype='labeled')
    fname = '%s_%s-%s.csv' % (stem, doctype, cohort_name)
    fpath = os.path.join(outputdir, fname)
    if kargs.get('save_', True): 
        print('makeMCSFromSource> Saving MCSs (cohort=%s, dim=%s) to:\n%s\n' % (cohort_name, str(dfmcs.shape), fpath))
        dfmcs.to_csv(fpath, sep='|', index=False, header=True, encoding='utf-8') 
    
    return

def makeMCSFromSource(**kargs):
    """
    Create MDSs for a given cohort from sequenced EHR data. 
    """
    def get_global_cohort_dir(): 
        return seqparams.getCohortGlobalDir(cohort_name) # basedir: sys_config.read('DataExpRoot')
    def summary_stats(): # <- all candidates 
        print('  + n_unique: %d' % len(allCandidates))  # 
        return 
    def is_id_consistent(idx1, idx2): 
        return len(idx1) == len(idx2) and len(set(idx1)) == len(set(idx2))
    def fill_labels(dfsrc, dfmcs, col='person_id'): # <- df, dfmcs
        dfsrc.sort_values(['person_id', ], ascending=True, inplace=True)[col].values
        dfmcs.sort_values(['person_id', ], ascending=True, inplace=True)[col].values
        dfmcs['label'] = dfsrc['NKF_Stage'] 
        return dfmcs
    def fill_labels_via_merge(dfsrc, dfmcs, col='person_id'):  # [debug]
        assert col in dfmcs.columns, "attribute %s does not exist in the MCS dataframe" % col

        # dfsrc: ['person_id', 'NKF_Stage', 'CaseControlUnknownStatus']
        # dfmcs:  ['person_id', 'sequence', 'timestamp', 'label']
        df = pd.merge(dfmcs, dfsrc, on=col, how='left')  # dfmcs's person_id could be null if no MCS found? 
        print('  + merged df header: %s' % df.columns.values)
        n1 = df.shape[1]
        # df.dropna()
        df = df.loc[~df['sequence'].isnull()]
        n2 = df.shape[0]

        # [error] Some patients do not have MCSs? n_src: 5 > n_mcs: 646232
        assert n1 == n2, "Some patients do not have MCSs? n_src: %d > n_mcs: %d" % (n1, n2)
        dfmcs[['sequence', 'timestamp', 'label']] = df[['sequence', 'timestamp', 'NKF_Stage']]    
    ...
    def test_mcs_integrity(dfmcs): # <- header
        # if header is None: header = TDoc.header_labeled_seq
        print('  + expected to contain header: %s' % header)
        assert all([h in dfmcs.columns for h in header])
        print('  + dfmcs dimension: %s' % str(dfmcs.shape))

        return

    import seqGen as smaker
    import seqparams
    from cohort_selection import ccs
    from tdoc import TDoc
    # import cohort.ccs as ccs
    ...
    header = TDoc.header_labeled_seq  # ['sequence', 'timestamp', 'label']
    header_src = ['person_id', 'NKF_Stage', 'CaseControlUnknownStatus']
    ...
    # labeled candidates  
    fname = 'CdwCkdCohort_20170817.csv'
    dfsrc = readAnnotatedDataSource(fname, cohort=cohort_name)  # load_labeled_candidates(suffix='query_ids-src')
    header_src = list(dfsrc.columns.values)

    # df.sort_values(['person_id', ], ascending=True, inplace=True)
    
    labeledPersonIDs = dfsrc['person_id'].values 
    nP0 = dfsrc.shape[0]

    # returned df has IDs: ['person_id', 'sequence', 'timestamp', ]
    dfmcs = createDocFromSequencedEHR(cohort=cohort_name, person_ids=labeledPersonIDs, groups=None)  # groups <- None to read all sequenced data
    # dfmcs.sort_values(['person_id', ], ascending=True, inplace=True)
    tIDConsistent = True if is_id_consistent(labeledPersonIDs, dfmcs['person_id'].values) else False 

    nP = dfmcs.shape[0]
    print('makeMCSFromSource> n_anno: %d =?= n_sequenced: %d' % (nP0, nP))
    ...
    if tIDConsistent: 
        ...
        # fill_labels_via_merge(dfsrc, dfmcs, col='person_id')
        fill_labels_via_lookup(dfsrc, dfmcs, col_id='person_id', col_label='NKF_Stage')
...
def createDocFromSequencedEHR(cohort, person_ids, **kargs):
    """

    Example 
    -------
    1. condition_drug_timed_seq-group-10.id  
       condition_drug_timed_seq-group-10.dat

    """
    def define_groups(n_parts=10):
        groups = kargs.get('groups', None)
        if groups is None: 
            print('  + use sequenced EHR data by default (n_parts=%d)' % n_parts)
            groups = [] 
        ...
        return groups
    def resolve_inputdir(): # <- inputdir
        inputdir = kargs.get('inputdir', None)  # tpheno/data-exp
        basedir_sequencing = os.path.join(sys_config.read('DataExpRoot'), 'sequencing')  # tpheno/data-exp/sequencing
        if inputdir is None: inputdir = basedir_sequencing
        assert os.path.exists(inputdir), "Invalid input dir: %s" % inputdir
        return inputdir
    def parse_timed_doc(group): # <- inputdir, stem, doctype, L
        fname_src = '%s_%s-%s.dat' % (stem, doctype, group)   # regular doc
        fpath = os.path.join(inputdir, fname_src)
        D, T = sr.readTimedDoc(fpath, simplify_code=False)  # params: simplify_code | test: max_doc, max_visit, verify_
        df = sr.readDocToDF(D, T=T)  # header = ['sequence', 'timestamp', 'label']
        return df
    def pack_ids(df, group): # <- inputdir, stem, doctype,
        fpath = os.path.join(inputdir, '%s_%s-%s.id' % (stem, doctype, group))
        df_idx = pd.read_csv(fpath, sep='|', header=0, index_col=False, error_bad_lines=True)
        assert df.shape[0] == df_idx.shape[0], "Dimension inconsistent > n_persons: %d but MCS has %d rows" % (df_idx.shape[0], df.shape[0])
        df['person_id'] = df_idx.values
        return

    import seqReader as sr
    from tdoc import TDoc

    inputdir = resolve_inputdir()
    stem, doctype = 'condition_drug', TDoc.doctypeToStem(doctype='time')
    target_cohort_name = cohort

    groups = define_groups()  # user-provided cohort (e.g. 'CKD') or sequenced EHR groups
    L = kargs.get('labels', [])
    # person_ids = kargs.get('person_ids', None)

    dfx = []
    header = ['sequence', 'timestamp', 'label', ]  # TDoc.header_labeled_seq
    for group in groups: # condition_occurrence-0.csv, drug_exposure-0.csv 
        fname_id = '%s_%s-%s.id' % (stem, doctype, group)   # dataframe
        fname_src = '%s_%s-%s.dat' % (stem, doctype, group)   # regular doc
        
        df = parse_timed_doc(group)
        pack_ids(df, group)  # add ids in 

        # filter 
        dfx.append(df.loc[df['person_id'].isin(person_ids)])

    df = pd.concat(dfx, ignore_index=True)
    # df.sort_values(['person_id', ], ascending=True, inplace=True)

    pid_src = person_ids
    pid_mcs = df['person_id'].unique() 
    pid_no_match = set(pid_src)-set(pid_mcs)
    print('createDocFromSequencedEHR> New MCS dimension: %s (cohort=%s) | n_matched: %d, n_no_data: %d' % \
        (str(df.shape), target_cohort_name, len(pid_mcs), len(pid_no_match)))

    # params: file naming
    # doctype = TDoc.doctypeToStem(doctype='labeled')
    # fname_new_src = '%s_%s-%s.csv' % (stem, TDoc.doctypeToStem(doctype='labeled'), target_cohort_name)

    # if kargs.get('save_', True): 
    #     df.to_csv(fp, sep=fsep, index=False, header=True, encoding='utf-8')
    return df  # ['person_id', 'sequence', 'timestamp', ]

def createDocFromDF(**kargs):
    """
    Create MCSs from the intermediate data (usually obtained from )

    Params
    ------
    condition_table: condition_occurrence-query_ids-augment.csv
    drug_table: e.g. drug_exposure-query_ids-augment.csv

    lab_table

    Use 
    ---
    t_augment_cohort(): augment existing cohort (e.g. CKD)
    ...
    def get_input_dir(): # where to expect the input table/dataframe? 
        ...
        return prefix_dir
    def manage_input():  # <- input_dir

        ret = {}
        ctb = df_condition = kargs.get('condition_table', DataFrame())  # 'condition_occurrence-query_ids-augment.csv'
        dtb = df_drug = kargs.get('drug_table', DataFrame())  # 'drug_exposure-query_ids-augment.csv'

        # inputdir = get_input_dir()
        if isinstance(ctb, str): 
            f_condition = os.path.join(inputdir, ctb)
            assert os.path.exists(f_condition), "Invalid path for condition table:\n%s\n" % f_condition

            dtypes = {'condition_source_value': str}
            df_condition = pd.read_csv(f_condition, 
                                        sep='|', index_col=False, header=0, error_bad_lines=True, 
                                        parse_dates=['condition_start_date'], dtype=dtypes) 
        if isinstance(dtb, str):
            f_drug = os.path.join(inputdir, dtb)
            assert os.path.exists(f_drug), "Invalid path for medication table:\n%s\n" % f_drug
    ...
    inputdir = get_input_dir()
    table = manage_input()
    df_condition = table.get('condition', DataFrame())
    df_drug = table.get('drug', DataFrame())
    df_lab = table.get('lab', DataFrame())

    for tstamp in (True, ): 
        ret = smaker.make_seq2(include_timestamps=tstamp,   # save_intermediate=True
                    include_diag=True, include_med=True, 
                    inputdir=inputdir,     # tpheno/data-exp/<cohort>
                    condition_table=df_condition,   # condition dataframe
                    drug_table=df_drug,        # drug dataframe
                    lab_table=df_lab,          # lab dataframe  ... usually empty
                    cohort=cohort_name, 
                    save_csv=True, save_id=True, # save .csv file and ID file 

                    # add this keyword to the stem of the file name (to distinguish from, say, original labeled cohort)
                    meta=kargs.get('meta', ''))  # e.g. 'augmented'
...
def t_sequencing(**kargs): 
    ...
    """
    # import seqMaker2 as smaker # older version of seqGen
    import seqGen as smaker
...
def t_verify_patient_set(cohort='CKD'):
    def load_source(sep=','): # <- fname, header, basedir
        # load annotated data, in which the labels (CKD stages) are determined by eMerge algorithms
        # e.g. eMerge_NKF_Stage_20170818.csv    ['patientId', 'Case_Control_Unknown_Status', 'NKF_Stage', ]
        #      CdwCkdCohort_20170817.csv     ['person_id', 'NKF_Stage', 'CaseControlUnknownStatus']
        ...
        df = pd.read_csv(fpath, sep=sep, header=0, index_col=False, error_bad_lines=True)
        return df
    def load_patient_ids(sep=',', col='person_id'): 
        fname = 'condition_drug_timed_seq-%s.id' % cohort
        fpath = os.path.join(basedir, fname)
        assert os.path.exists(fpath), "Invalid source directory: %s" % fpath
        df = pd.read_csv(fpath, sep=sep, header=0, index_col=False, error_bad_lines=True)
        return df[col] 
    ...
    ### annotated source
    basedir = seqparams.getCohortGlobalDir(cohort)
    fname = 'CdwCkdCohort_20170817.csv'
    id_field = 'person_id'
    # diagCodes = ccs.getCohort(cohort_name)
    # CKD_codes = ['585', '585.1', '585.2', '585.3', '585.4', '585.5', '585.6', '585.9', '792.5', 'V42.0', 'V45.1', 'V45.11', 
    ...
    # labeled candidates  
    dfsrc = load_source() # readAnnotatedDataSource(fname, cohort=cohort_name)  # load_labeled_candidates(suffix='query_ids-src')
    pidx1 = dfsrc[id_field].values

    ### documents: ID files
    pidx2 = load_patient_ids()
    print('verify: ID(source): %d , ID(id): %d' % (len(pidx1), len(pidx2), all(pidx1==pidx2)))

    ### documents: MCS 
    ifiles = ['condition_drug_labeled_seq-CKD.csv', ]
    D, L, T = docProc.processDocuments(cohort=cohort, inputdir=basedir, ifiles=ifiles, seq_ptype='regular', 
                                doc_filter_policy='minimal_evidence', min_ncodes=1)  # 'source_type', 'slice_policy'
    print('verify> n_persons: source %d, mcs: %d' % (len(id_field), len(D)))
...
def generate(**kargs): 
    cohort_name = kargs.get('cohort', 'CKD')
    src = kargs.get('source', 'intermmediate') # options: 'sequencing', 'intermmediate'

    if src.startswith('seq'): 
        makeMCSFromSource(cohort=cohort_name)  # create MCS from sequenced EHR at tpheno/data-exp/sequencing
    else:  # intermmediate
        makeMCS(cohort=cohort_name)  # create cohort-specific MCSs from intermediate files (dataframes curated from ODHSI DB)
    
    # todo
    print('generate> Create labeled MCSs ...')
    makeLabeledSeq(cohort=cohort_name, annotated_file=kargs.get('annotated_file', 'CdwCkdCohort_20170817.csv'))  

    return

def test(**kargs): 
    """
    New tests for collecting targeted cohorts. 

    Memo
    ----
    1. eMerge annotated CKD cohort
       eMerge_NKF_Stage_20170818.csv    ['patientId', 'Case_Control_Unknown_Status', 'NKF_Stage', ]
       CdwCkdCohort_20170817.csv     ['person_id', 'NKF_Stage', 'CaseControlUnknownStatus', ]
    """
    
    ### 1. Cohort Definition
    # <workflow> e.g. 1) run t_search_ckd() to collect intermediate dataframes 2) generate()
    # t_search_ptsd(**kargs)

    # the t_search_<disease> routine produces intermediate files 
    annotated_file = 'CdwCkdCohort_20170817.csv'
    t_search_ckd(annotated_file=annotated_file)  # to be used later on in seqGen to make per-patient coding sequences
    
    ## 1a. Generate MCSs for a given disease cohort
    generate(cohort='CKD', source='intermmediate', annotated_file=annotated_file)  # [todo]
    # makeMCS(cohort='CKD')
    # makeMCSFromSource(cohort='CKD')  # create MCS from sequenced EHR at tpheno/data-exp/sequencing
    # makeLabeledSeq(cohort='CKD')  

    # 1b. check label
    # t_getLabels(cohort='CKD')  # labeled data

    # 1c. make training set
    # make_tset()
    ...
    ### 2a Create augmented source documents (in addition to labeled candidates)
    ...
    ### 3. Cohort summary 
    ifiles = ['condition_drug_timed_seq-CKD.csv', ]
    # summarizeCohort(cohort='CKD0', inputdir=None, ifiles=ifiles, seq_ptype='regular', **kargs)

    ### 4. Verify Cohort 
    
    # annotated source vs MCS
    # t_verify_patient_set(cohort='CKD')
