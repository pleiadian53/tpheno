...
def deriveLCS2(D, **kargs):
    ...
    def filter_by_policy(lcsmap, topn=100, min_df=2, reverse=True): # params: lcsmap
        ...
        if policy.startswith('u'):  # uniqnuess
            lcsmap = filter_lcs_by_uniq(lcsmap, topn=topn, min_df=min_df) # select top n LCSs, all of which occurr in at least m documents 
        elif policy.startswith('d'):  # df, document frequency
            lcsmap = filter_lcs_by_docfreq(lcsmap, topn=topn, min_df=min_df, reverse=reverse)
        elif policy == 'noop': 
            pass
        else: 
        ...
        print("deriveLCS2.filter_by_policy> lcsmap size %d -> %d | policy=%s, topn=%d, min_df=%d" % \
            (N0, len(lcsmap), policy, topn, min_df))
        return lcsmap
    def analyze(): 
        # [note] design 
        T = kargs.get('T', [])
    ...
    minLCSDocFreq = 2     # min(len(D)/10, 10)
...
def deriveLCS(D, **kargs):
    ...
    for setting in settings: 
        ...
        lcsmap = filter_by_policy(lcsmap, topn=topn_lcs, min_df=minNDoc, reverse=reverse)  # option: lcs_policy
...
def loadLCSFeatureTSet(**kargs):
    ...
    def modify_tset(X, y): 
        ...
        if focused_classes is not None: # only preserve these classes and everything else becomes "Others"
            ...
            y = sclf.focusLabels(y, labels=focused_classes, other_label='Control')  # output in np.ndarray
...
def makeLCSFeatureTSet2a(cohort, **kargs):  # [note] maybe do away with the explicit arg: cohort
    ...
    if len(lcsCandidates) == 0: 
        # params: cohort, lcs_type, lcs_policy, ctype, consolidate_lcs, label, meta
        df = lcsHandler.load_lcs(lcs_type=kargs.get('lcs_type', 'global'))  # load the global LCS training set by default (use None as default)
        assert df is not None and not df.empty, "Could not load pre-computed LCS feature set."
    ...
    # policy A: rank feature by specified criteria e.g. term frequency
    topNFSet = kargs.get('topn', None)
    df = load_fset(topn=topNFSet) if kargs.get('load_fset', True) else None # params: 'policy_fs' (sorted, tfidf, tf, ... )

    tUseRareLCS = kargs.get('use_rare_lcs', True)
    minDocFreq = 2
    if df is None or df.empty: 
        # params: 'policy_fs'
        print('makeLCSFeatureTSet2a> creating a new set of ranked LCS features ...')
        df = rank_fset(lcsCandidates, save_=True, topn=topNFSet, policy_fs='sorted') # alphanumeric sorted order
    else: 
...
def makeLCSFeatureTSet3(cohort, **kargs): 
    ...
    def eval_feature_value(D, L, fset, lcs_result, inputdir=None, content_sep=','): 
        ...
        # ... then check docID -> {(lcs, freq)}
        gTermFreq = lcs_result['frequency']    # initvarcsv(name='lcsmapInvFreq')  # id -> {(lcs, tf)}
    ...
    dfG = lcsHandler.load_lcs()  # local global LCS candidates 
    n_sample = dfG.shape[0]
    max_n_samples = min(30000, dfG.shape[0]) 
    ...
    print('  + Selected %d (from %d) global LCS canidates ...' % (len(lcsCandidates), dfG.shape[0]))  # [note] got 43406 as of 07.18
    ...
    n_features = kargs.get('topn', 10000)
    lcsCandidates = [f for f, cnt in sorted(candidates, key=lambda x: x[1], reverse=False)[:n_features]] # least frequent
    print('  + got %d least freq LCS features | min df: %d' % (len(lcsCandidates), minLCSDocFreq))

    # sparse format by default 

    lcsFSet = sorted(lcsCandidates)
    Xs, Ls = eval_feature_value(D, L, fset=lcsFSet, lcs_result=ret)  # feature values are not scaled
    assert Xs is not None, "Could not compute feature values"
    if kargs.get('scale_', False): # could destroy sparsity 
        print('  + scaling feature values (z-scores)')
        std_scaler = StandardScaler(with_mean=False)
...
def makeLCSTSet(cohort, **kargs):
    ...
    def load_lcs(): # lcs_type
        ltype = kargs.get('lcs_type', lcsHandler.lcs_type)  # global: Common LCSs defined in global scope (as opposed to cluster or local scopes)
...
def t_analyze_lcs(**kargs):
    def load_lcs(): # lcs_type
        ltype = kargs.get('lcs_type', lcsHandler.lcs_type)  # global: Common LCSs defined in global scope (as opposed to cluster or local scopes)
        lcs_select_polcy = kargs.get('lcs_policy', lcsHandler.lcs_policy)
        # seq_ptype = kargs.get('seq_ptype', 'regular')
...
def t_lcs(**kargs): 
    ...
    def analyze_the_other_strata(current_label, df_lcs, save_=True):  
        ...
        for label, entry in stratified.items(): 
            ...
            # query frequency: number of docs in the other strata in which lcs occurs
            # [note] candiate attributes: ['length', 'lcs', 'n_uniq', 'count', 'df', ]
            ret = analyzeMCS(Di, Ti, lcs_set=lcs_set, make_color_time=False)  # index:lcsmap, frequency:lcsmapInvFreq
        ...
        if save_: save_relative_stats(df_rel, label)
        return df_rel
    ...
    for label, entry in stratified.items():
        ...
        df = deriveLCS(D=Di, 
                    ...
                    topn_lcs=kargs.get('topn_lcs', topNLCS), # "topN" for enriched and rare
                    min_length=kargs.get('min_length', minLength), max_length=kargs.get('max_length', maxLength), 
                    min_ndocs=kargs.get('min_ndocs', minLCSDocFreq),  # depends on the size of D
                    max_n_pairs=kargs.get('max_n_pairs', maxNPairs),  
                    remove_duplicates=True,
        ...
        df = sample_lcs_set(df, max_n=topNLCS, pattern_type='enriched', criteria=['count', ])
        ...
        analyze_the_other_strata(current_label=label, df_lcs=df, save_=True)
...
def t_lcs2(**kargs):
    ...
    def get_tset_dir(): # derived MDS 
        ...
        return tsetPath 
    def analyze_strata(stratified, current_label, lcs_candidates, topn=100, raren=100, save_=True, max_n_samples=None):  
        # df_lcs: dataframe returned from deriveLCS() 
        #         columns: ['length', 'lcs', 'n_uniq', 'count', 'df', ]
        ...
        Tr = entry['timestamp'] 
        Lr = entry['label']  
        ret = analyzeMCS(Dr, Tr, lcs_set=lcs_candidates, make_color_time=False)  # index:lcsmap, frequency:lcsmapInvFreq
        ...
        for setting in settings: 
            ...
            df_rel = DataFrame(adict, columns=header)
            if save_: save_relative_stats(df_rel, label, meta=meta)
    ...
    def save_relative_stats(df, label, meta='enriched'):  
        ...
        suffix = '%s-L%s' % (meta, label)         
        # fpath = Pathway.getFullPath(cohort=cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix) # [params] dir_type='pathway' 
        print('t_lcs2> saving LCS labels ...')
        Pathway.save(df, cohort=cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix, dir_type='pathway')
        return 
    ...
    for i in range(nTrials): 
        ...
        df, summary = docToLCS(lcsHandler.cohort, 
                            ...
                            load_lcs=tLoadLCS, # False => recompute a new set of LCS
                            include_docs=False, 
        ...
        print('  + LCS computaiton complete => size of df=%d [n_trial=%d]' % (df.shape[0], i))
    df = None; gc.collect()
    ...
    lcsCandidates = dfG.sample(n=max_n_samples)['lcs'] # dfG['lcs'].values
    print('  + Found %d global LCS canidates ...' % dfG.shape[0])  # [note] got 43406 as of 07.18
    ...
    for label, entry in stratified.items():
        ...
        analyze_strata(stratified, current_label=label, lcs_candidates=lcsCandidates, topn=topNLocal, raren=rareNLocal, save_=True) 
...
def t_make_lcs_fset(**kargs):
    ...
    if tCompleteData: 
        if tMakeTSet:
            ...
            # select least frequent LCSs 
            X, y = makeLCSFeatureTSet3(cohort, **kargs)  # if non-incremental, use makeLCSFeatureTSet2(cohort, **kargs) 
            summary()
...
def t_classify(**kargs):
    ...
    maxSampleRatios = kargs.get('ratios', [0.2, 0.1, ])
...
def t_lcs_feature_select(**kargs):
    """
    Use statistical models for feature selection. 
    """
