...
#  Reference
#  --------- 
#  Derived from qrymed2 for stand-alone use. 
#
#
########################################################################

# [old] module-dependent path: sys_config.read('RefDir')
RefDir = os.path.join(os.getcwd(), 'ref') # sys_config.read('RefDir')  # external support data (e.g. ICD9_descriptions)

# old path: sys_config.read('bin') 
BinDir = os.path.join(os.getcwd(), 'bin') # set this path to point to where the original 'qrymed' (in Perl) is kept

# [old]
# ICD9Tbf = os.path.join(RefDir, 'ICD9_descriptions') # used for reverse lookup from ICD9 codes to MED codes
...
def _exec2(cmd, err_default=None, verbose=True):
    ...
    if st != 0: 
        ...
        if verbose: print('error> input cmd:\n%s\n' % cmd)
        return (st, output)
    return (st, output) 

def _htemplate(switch, _self=True, expand=False):
    switch = switch    
    if _self: switch = 'i%s' % switch
...
def _query_code(switch, args, to_int=True): 
    cmd = 'qrymed -%s %s' % (switch, args)
    codes = _exec(cmd); codes = codes.split()
    if codes and to_int:
...
def getName(codes, **kargs):
    cmd = 'qrymed -pnm %s' % codes 
    return _exec2(cmd, **kargs)

def getName2(codes, **kargs): 
    # print('debug> codes: %s' % codes)
    st, output = getName(codes, **kargs)
...
def getDescendent(code, expand=False, _self=False, 
                  write_tofile_=False, file_=None, to_int=True):
    switch = 'desc'    
    if _self: switch = 'idesc'
    if not expand: 
        return _query_code(switch, code, to_int=to_int)     
    cmd += ' | ' + 'qrymed -e -pnm'
    return _exec(cmd)

def idesc(code, **kargs): 
    return getDescendent(code=code, _self=True, **kargs)
def desc(code, **kargs): 
    return getDescendent(code=code, _self=False, **kargs)

def getAncestor(code, expand=False, _self=True, 
                  write_tofile_=False, file_=None, to_int=True):
    switch = 'anc'    
    if _self: switch = 'ianc'
    if not expand: 
        return _query_code(switch, code, to_int=to_int)    
    cmd += ' | ' + 'qrymed -e -pnm'
    return _exec(cmd)
...
def ianc(code, **kargs): 
    return getAncestor(code=code, _self=True, **kargs)
def anc(code, **kargs): 
    return getAncestor(code=code, _self=False, **kargs)
...
def _ntemplate(name='microbiology', postfix='desc', _getter=None, use_table=False, 
    ...
    tb = {}
    outputdir = kargs.get('outputdir', os.path.join(os.getcwd(), 'data-exp'))
    ...
    for code in codes:
        codeset = _getter(code, expand=False, _self=_self) 
        ...
        if to_int:
            ...
            tb[code] = codeset  
    if save_: 
        f = os.path.join(outputdir, '%s_%s.pkl' % (name, posfix)) # [todo]
        pickle.dump(desctb, open(f, "wb" ))         
    return tb

def getAncestorByName(name='microbiology', use_table=False, dup=False, save_=True, _self=True, to_int=True):  
    tb = _ntemplate(name=name, posfix='anc', _getter=getAncestor, dup=dup, 
                       save_=save_, _self=_self, to_int=to_int)
    return tb

def getDescendentByName(name='microbiology', use_table=False, dup=False, save_=True, _self=False, 
    ...
    for code in codes:
        ...
        #     desctb[code] = []
        codeset = set(getDescendent(code, expand=False, _self=_self, to_int=to_int))
        if verbose > 2: print "desc> %s has %s descendents ->\n   %s" % (code, len(codeset), list(codeset)[:10])
        desctb[code] = codeset  
    if save_: 
        f = os.path.join(outputdir, '%s_desc.pkl' % name)
        pickle.dump(desctb, open(f, "wb" ))
    return desctb
def codePair(desctb): 
    heads, members = [], set([])
...
def getDescendentByName2(name='microbiology', dup=False, save_=True, _self=False, 
...
def getDescendentHierarchy(name='microbiology', save_=True, load_=False):
    ...
    if not desctb: 
        desctb = getDescendentByName(name=name, _self=False, 
    ...
    for head, members in desctb.items(): 
        ...
        while mx: 
            m = mx.popleft()
            dmx = set(getDescendent(m, _self=False, to_int=True))
            if dmx: 
                overlap = dmx.intersection(mx)
    ...
    if save_: 
        f = os.path.join(outputdir, '%s_hierarchy.pkl' % name)
        pickle.dump(desctb, open(f, "wb" ))
    return desctb  
...
def save(obj, file_=None, labtest='microbiology', typ='desc', verbose=True): 
    outputdir = kargs.get('outputdir', os.path.join(os.getcwd(), 'data-exp'))
    if not os.path.exists(outputdir): os.makedirs(outputdir) # base directory
    ...
    if file_ is None: 
        ...
        f = f % {'root': outputdir, 'name': labtest}
    else: 
        dataroot, f = os.path.dirname(file_), os.path.basename(file_)
        if not dataroot: dataroot = outputdir
        f = os.path.join(dataroot, f)
    pickle.dump(obj, open(f, "wb" ))
    if verbose: print "save> input saved to %s" % f
    return 

def load(file_=None, labtest='microbiology', typ='desc'): 
    outputdir = kargs.get('outputdir', os.path.join(os.getcwd(), 'data-exp'))
    if not os.path.exists(outputdir): os.makedirs(outputdir) # base directory
...
def filterByName(name='microbiology', load_=False, to_int=True, desc=True):
    ...
    if load_: 
        ...
        f = "%s/%s_%s.pkl" % (outputdir, name, 'desc' if desc else 'anc')
        try: 
    ...
    if not has_data:
        if desc:  
            tb = getDescendentByName(name, _self=True, dup=False, to_int=to_int)
        else: 
            tb = getAncestorByName(name, _self=True, dup=False, to_int=to_int)
...
def test2(): 
    # kargs = {'to_int': False} 
    # codes = getDescendent(code=1000, _self=True, **kargs)
...
def t_map(): 
    ...
    code = '35789'
    cmd = 'qrymed -val 16 %s' % code  # e.g. 35789     slot 16: entity measured

    output = _exec2(cmd, err_default=None) 
    print(output)
