...
def run_tsne2(X, y=None): 
    # from sklearn.manifold import TSNE
    RS = 20170322
    X_proj = TSNE(random_state=RS).fit_transform(X)
    return X_proj 

def run_tsne(X, y=None, **kargs):
    # from tsne import bh_sne

    # [params]
    plot_result = kargs.get('plot_', True)
    basedir = kargs.get('outputdir', os.path.join(os.getcwd(), 'plot')) # sys_config.read('')
    if not os.path.exists(basedir): os.mkdir(basedir)
    meta = kargs.get('identifier', 'sprint')
    ...
    plt.clf()
    if plot_result: 
        plt.scatter(x0, x1, c=y, cmap=plt.cm.get_cmap("jet", 10))    	
...
def run(X=None, y=None, **kargs): 
    ...
    if ts is None or ts.empty: 
        ts = sc.load_tset(**kargs)

    # Xa, ya, D = sc.build_data_matrix2(**kargs)
    
    X, y = evaluate.transform(ts, standardize_='minmax')  # defined in evaluate
    print('proposal> feature set has %d-D' % X.shape[1]-1)

    n_uniq_labels = len(set(y))

    # verifying the data
    print('run> X (dim: %s), y (dim: %s)' % (str(X.shape), str(y.shape)))  
    print('verify> example composite labels:\n%s\n' % y[:10])
    
    print('verify> total number of labels: %d > n_uniq_labels: %d' % (len(y), n_uniq_label))
