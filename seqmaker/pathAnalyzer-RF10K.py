...
def featureSelectLasso(topn, **kargs): 
    """
    Select features based on importance weights derived from LASSO. 
    """
    def load_tset(n_per_class=None): 
        X, y = kargs.get('X', None), kargs.get('y', None)
        if X is None or y is None: 
    ...
    def select_features(X, y, n=10000, fset=None, policy_fs='sorted', 
                estimator=None, n_sample_per_class=3000, threshold=None, n_iter=10):  # <- ts
        ...
        active_lcsx, positions = evaluate.select_features(X, y, 
            ...
            n_per_class=n_sample_per_class, threshold=threshold, estimator=estimator)  # threshold 1e-5 by default if penalty='l1'
        displayMax = 100
        for i, lcs in enumerate(active_lcsx): 
            if i < displayMax: 
                print('  + [#%d, pos=%d] %s' % ((i+1), positions[i], lcs))
    ...
    lcsFSet, pos = select_features(X, y, n=topn, fset=lcsFSetTotal, 
            ...
            n_sample_per_class=maxNSamplePerClass, n_iter=nIterFS)    
...
def t_classify(**kargs):
    ...
    if ts_dtype.startswith('d'): 
        ...
        if kargs.get('apply_fs', True): 
            X, y, fset = featureSelectLasso(topn=topNFSet, X=X, y=y, policy_fs='sorted', 
...
def test(**kargs): 
    ...
    identifier = 'Flcs10K-RF' # use this instead of userFileID for now, feature selection
    cohort = kargs.get('cohort', 'CKD')
    # use 'regular' for creating feature set but separate 'diag' and 'med' when doing LCS analysis
    ctype = seqparams.normalize_ctype('regular')  # 'diag', 'med'
    sq.sysConfig(cohort=cohort, seq_ptype=ctype, 
    ...
    maxNSamplePerClass = 3000   # feature selection
    topNFSet = 10000 # number of features (upon feature selection)
