...
def pClusterPathwayLCSTimeSeries(**kargs): 
    ...
    # set load_ to True to load processed sequences (otherwise the temporal documents will be made on the fly)
    if read_from_source: 
        # print('input> reading from source temporal doc file: %s' % ifile)
        # documents = sa.read(load_=False, simplify_code=False, mode=read_mode, verify_=False, 
        #         seq_ptype=seq_ptype, ifile=ifile, cohort=cohort_name)  # # values: regular, random, diag, med, lab ... default: regular
        res = sr.readToCSVDoc(load_=False, simplify_code=False, 
                                verify_=True, include_timestamps=True, 
                                seq_ptype=seq_ptype, cohort=cohort_name)
        assert not include_timestamps or len(res) == 2
        if include_timestamps: 
            documents, timestamps = res 
        ...
        ### documents and timestamps read directly from the dataframe are of "string" types and need to be parsed. 
        # tdoc_prefix = seq_compo
        # tdoc_stem = 'timed_seq'
        seq_ptype_prime = 'regular'
        f_tdoc = '%s_%s-%s-%s.csv' % (tdoc_prefix, tdoc_stem, cohort_name, seq_ptype_prime)  # e.g. condition_drug_timed_seq-PTSD-regular.csv
        ...
        assert os.path.exists(fpath_tdoc), 'Invalid input path: %s' % fpath_tdoc
        df_seq = pd.read_csv(fpath_tdoc, sep='|', header=0, index_col=False, error_bad_lines=True)
        documents = list(df_seq['sequence'].values)
        timestamps = list(df_seq['timestamp'].values)
