...
Q_Join = """
SELECT c.person_id, c.condition_start_date as start_date, d.drug_exposure_start_date as start_date, condition_concept_id as concept_id, 
FROM Production.Product p
JOIN Purchasing.ProductVendor pv
ON p.ProductID = pv.ProductID
JOIN Purchasing.Vendor v
...
def rangeQuery(statement=None, **kargs):
    ...
    for i in range(n_datasets): # (0, ):  # set index set here
        ...
        for tb, qtemp in tmap.items():  # foreach table and query template
            ...
            query(statement=q, identifier=filename, save_intermediate=True, index=i)  # this saves intermediate data for make_seq()
...
def query(statement, **kargs):  
    ...
    Params
    ------
    save_intermediate (set to true to save output)
    ...
    if save_intermediate:   # get df: TDoc.getSourceTable(identifier, index)  # index is ignored if set to None 
        index = kargs.get('index', None)
        basename = kargs.get('identifier', "sequencing")
...
def fetch(**kargs):
    """
    Fetch all pateint data. 
    """
    def person_ids(col='person_id'): # df_condition, df_drug 
        idx1 = df_condition[col].unique()
        idx2 = df_drug[col].unique()
        return list(merge(idx1, idx2))

    from heapq import merge  # merge two sorted list

    tb_condition = 'condition_occurrence'
    tb_drug = 'drug_exposure'
    q_condition = """select person_id, condition_start_date as start_date, condition_concept_id as concept_id
                     from ohdsi.west.condition_occurrence
                     order by person_id, start_date"""
    q_drug = """select person_id, drug_exposure_start_date as start_date, drug_concept_id as concept_id
                from ohdsi.west.drug_exposure 
                order by person_id, start_date"""

    df_condition = query(statement=q_condition, identifier='condition_occurrence_all', save_intermediate=True) # [params] index, outputdir
    print('fetch> found %d rows in table %s' % (df_condition.shape[0], tb_condition))
    df_drug = query(statement=q_drug, identifier='drug_exposure_all', save_intermediate=True)
    print('fetch> found %d rows in table %s' % (df_drug.shape[0], tb_drug))
...
def fetch0(**kargs):
    ...
    2. old version from seqMaker2 specifically for diabetes cohort
    """
    def save_part(df, tb, index=0): 
        fp = os.path.join(temp_dir, '%s-%d.csv' % (tb, index))  # e.g. measurement-153.csv
        if not os.path.exists(fp) or (os.path.exists(fp) and overwrite_intermediate): 
...
def parse(coding_seq, time_sep='|', token_sep=',', token_visit=';', token_record='$', check_time=False):
    ...
    """ 
    assert coding_seq[-1] == token_record, "ill-formatted coding sequence:\n%s\n" % coding_seq[-10:]
    seq = coding_seq[:-1]
    
    codeSeq, timeSeq = [], []
    visits = [visit for visit in seq.split(token_visit) if len(visit) > 0] # remove empty strings 
    n_visits = len(visits)
...
def separate_time2(seq, time_sep='|', token_sep=',', token_visit=';', token_record='$', check_time=False): 
    ...
    cstr, tstr = assemble(cseq, tseq, token_sep=token_sep, token_visit=token_visit, token_record=token_record, for_csv=True)
    return (cstr, tstr)
...
def make_seq(**kargs): 
    ...
    # fp_measure = '%s.csv' % tb_measure # measurement
    ...
    # [params] input dataframes (use fetch() or cohort.py to generate)
    condition_tb = kargs.get('condition_table', fp_condition)  # default diabetes cohort (determined via CCS: Clinical Classification Software)
    drug_tb = kargs.get('drug_table', fp_drug)

    # necessary columns
    col_condition = 'condition_concept_id' # 'condition_source_value'
    col_drug = 'drug_concept_id'           # 'drug_source_value'
    col_measurement = 'measurement_concept_id'
    header_condition = ['person_id', 'condition_start_date', col_condition, ]
    header_drug = ['person_id', 'drug_exposure_start_date', col_drug, ]
    header_lab = ['person_id', 'measurement_date', 'measurement_time', 'value_as_number', col_measurement]
    
    # condition header: person_id|condition_start_date|condition_source_value
    dtypes = {col_condition: int}
    ...
    f_tdoc_csv = '%s_%s.csv' % (tdoc_prefix, f_base)  # [output]
    ...
    for e in df_drug[col_drug].values: 
        if e == 0: # unmapped (a lot of them!)
    ...
    # diagnostic codes
    noncoded_condition = {}
    n_unmapped_condition = 0 
    for e in df_condition[col_condition].values:
        if e == 0: 
            n_unmapped_condition += 1
    ...
    for ith, pid in enumerate(person_ids):  # foreach person ids (sorted in ascending order)
        ...
        if include_diag: 
            ...
            if not df.empty: 
                ...
                df[d_col] = df[col_condition]  # condition_concept_id
                df = df[[t_col, d_col]]
                has_diag = True
...
def makeSeq(person_ids, datasets, **kargs):
    """
    
    Params
    ------
    dataframes: a list or a dictionary: table_name -> (intermmedicate) dataframe
    """
    def normalize_dataframes(): 
        if isinstance(dataframes, list): 
            datasets = {i: df for i, df in enumerate(datasets)}
        assert isinstance(datasets, dict)
        return datasets

    basedir = kargs.get('prefix', os.path.join(sys_config.read('DataExpRoot'), 'sequencing'))
    fpath_condition = os.path.join(basedir, )
...
def t_sequencing(**kargs): 
    ...
    # print('query>\n%s\n' % q)
    # query(statement=q, identifier='test', save_intermediate=True)
