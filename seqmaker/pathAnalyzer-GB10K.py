...
def featureSelect(topn, **kargs): 
    ...
    if kargs.get('load_selected', False):
        ...
        if topn == len(lcsFSet): 
            ...
            print('featureSelect> precomputed high-ranked features has a different size: (%d) but requested %d' % (len(lcsFSet), topn))
    ...
    print('featureSelect> size(total): %d, size(selected): %d =?= requested: %d' % (len(lcsFSetTotal), len(lcsFSet), topn))
...
def t_classify(**kargs):
    ...
    if ts_dtype.startswith('d'): 
        ...
        # [note] request 'topNFSet' LCS features but may not actually get that many
        ...
        result_set = []
        for clf in clf_list: 
            res = sclf.multiClassEvaluateSparse(X=X, y=y, cohort=lcsHandler.cohort, seq_ptype=lcsHandler.ctype, # not needed 
                    classifier=clf, 
...
def test(**kargs): 
    ...
    identifier = 'Flcs10K-GBT' # use this instead of userFileID for now, feature selection
    cohort = kargs.get('cohort', 'CKD')
    # use 'regular' for creating feature set but separate 'diag' and 'med' when doing LCS analysis
    ctype = seqparams.normalize_ctype('regular')  # 'diag', 'med'
    sq.sysConfig(cohort=cohort, seq_ptype=ctype, 
    ...
    aucs = t_classify(identifier=identifier, 
               clf_name='gbt', n_estimators=nTrees, # classifier
               n_per_class=maxNPerClass,  # data for classification
