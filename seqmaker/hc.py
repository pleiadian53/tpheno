...
def fancy_dendrogram(*args, **kwargs):
    ...
    if not kwargs.get('no_plot', False):
        ...
        plt.xlabel('sample labels', fontsize=12)
...
def p_dendrogram(Z, y=None): # p: plot 
    ...
    dendrogram(
            Z,
            labels = y, 
            leaf_rotation=27.,  # rotates the x axis labels
...
def p_truncated_dendrogram(Z, **kargs):
    # [params]
    basedir = os.path.join(os.getcwd(), 'plot') # sys_config.read('')
    if not os.path.exists(basedir): os.mkdir(basedir)
    ...
    if not kargs.has_key('descriptor'): 
        kargs['descriptor'] = 'test'
    ext = 'tif'
...
def p_dendrogram2(Z, **kargs):
    ...
    title = 'Metric: %s, D2V: %s, Sequence: %s' % \
        (kargs.get('metric', '?'), kargs.get('d2v_method', '?'), kargs.get('seq_ptype', '?'))
    ...
    fancy_dendrogram(
        ...
        annotate_above=9,  # useful in small plots so annotations don't overlap
    ...
    if not kargs.has_key('descriptor'): 
        kargs['descriptor'] = '%s_%s' % ('hc', graph_mode)

    ext = 'tif'
    fname = seqparams.name_cluster_file(**kargs) # '%s_%s.%s' % (descriptor, graph_mode, ext) 
    fpath = os.path.join(basedir, fname)
...
def run(X, y, D, **kargs): 
    ...
    metric_default = 'euclidean'  # cosine, hamming, jaccard, mahalanobis, minkowsk, correlation, seuclidean (standardized euclidean)
    metrics = kargs.get('metrics', kargs.get('metric', ['euclidean', 'seuclidean', 'cosine', 'correlation', ])) 
    ...
    for metric in metrics: 
        ...
        try: 
    	    p_dendrogram2(Z, y=y, last_n_merge=50, max_d=None, **kargs)
