...
def make_s_curve(**kargs): 
    ...
    ext = 'tif'
    fpath = os.path.join(TestDir, seqparams.name_image_file(descriptor='scurve', **kargs))
...
def locally_linear(X, **kargs): 
    ...
    for i, method in enumerate(methods):
        ...
        # scatter2(X_proj, y)
        fpath = os.path.join(TestDir, seqparams.name_image_file(descriptor=method, **kargs)) # kargs: seq_ptype, d2v_method
        plt.savefig(fpath, dpi=300)
        plt.close()
    return 
...
def isomap(X, **kargs): 
    ...
    fpath = os.path.join(TestDir, seqparams.name_image_file(descriptor='isomap', **kargs)) # kargs: seq_ptype, d2v_method
...
def mds(X, **kargs):
    ...
    plot_mode = kargs.get('graph_mode', 'seaborn')
    fpath = os.path.join(TestDir, seqparams.name_image_file(descriptor='mds', **kargs)) # kargs: seq_ptype, d2v_method
...
def spectral(X, **kargs):
    ...
    plot_mode = kargs.get('graph_mode', 'seaborn')
    fpath = os.path.join(TestDir, seqparams.name_image_file(descriptor='spectral', **kargs)) # kargs: seq_ptype, d2v_method
...
def tsne(X, **kargs): 
    ...
    # [output]
    fpath = os.path.join(TestDir, seqparams.name_image_file(descriptor='tsne-1', **kargs)) # kargs: seq_ptype, d2v_method
    ...
    if plot_mode.startswith('s'):
        fpath = os.path.join(TestDir, seqparams.name_image_file(descriptor='tsne-s', **kargs)) # kargs: seq_ptype, d2v_method
        scatter2(X_proj, y)
        plt.savefig(fpath, dpi=300)
        ...
        fpath = os.path.join(TestDir, seqparams.name_image_file(descriptor='tsne-r', **kargs)) # kargs: seq_ptype, d2v_method
        plt.savefig(fpath, dpi=300)
