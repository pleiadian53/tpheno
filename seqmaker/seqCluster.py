...
def makeTSet(**kargs): # same as seqClassify.makeTSet but does not distinguish train and test split in d2v
    """
    ...
    return seqClassify.makeTSet(**kargs) 
...
def evalMotif(**kargs): 
    return deriveNGram(**kargs)
def deriveNGram(D, topn=100, min_length=1, max_length=8, partial_order=False, **kargs): 
    """
    Given input corpous (D), find top n (topn) frequent n-grams with lengths within (min_length, max_length)


    Note
    ----
    1. This is parallel to pathwayAnalyzer.deriveLCS()
    ...
    Memo
    ----
    1. LCS/pathway file naming convention
          pathway_global-df-diag-iso-LCKDStage3a.csv 
    ...
    def do_slice(): # only for file ID, see secondary_id()
        ...
        if kargs['slice_policy'].startswith(('noop', 'reg', 'compl', )):  # effective operations; noop, regular, complete
            return False 
        return True
    def secondary_id(): # attach extra info in suffix
        ...
        if kargs.get('consolidate_ngram', True): suffix = '%s-%s' % (suffix, 'iso') # Ngrams up to permutations consider as the same? 
        ...
        return suffix 
    def lcs_file_id(): # params: {cohort, ng_type, ng_policy, seq_ptype, slice_policy, consolidate_ngram, length}
        ...
        adict['ngram_type'] = ltype = kargs.get('ng_type', 'global')  # global: Common LCSs defined in global scope (as opposed to cluster or local scopes)
        adict['ngram_policy'] = kargs.get('ng_policy', 'freq') # definition of common LCSs

        # use {seq_ptype, slice_policy, length,} as secondary id
        adict['suffix'] = adict['meta'] = suffix = secondary_id()
        return adict
    def load_ngram(): # [note] for now, only used in makeLCSTSet()
        adict = lcs_file_id()
        cohort = adict['cohort']
        ng_type, ng_policy = adict['ng_type'], adict['ng_policy']
        suffix = adict['meta'] 
        return Pathway.load(cohort, scope=ng_type, policy=ng_policy, suffix=suffix, dir_type='pathway', pattern_type='ngram')
    def save_ngram(df):  
        adict = lcs_file_id()
        cohort = adict['cohort']
        ng_type, ng_policy = adict['ngram_type'], adict['ngram_policy']
        suffix = adict['meta']         
        # fpath = Pathway.getFullPath(cohort=cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix) # [params] dir_type='pathway' 
        print('  + saving Ngrams ...')
        Pathway.save(df, cohort=cohort, scope=ng_type, policy=ng_policy, suffix=suffix, dir_type='pathway', pattern_type='ngram')
        return
    def update(ng_ith_doc, ng_total):
        # print('> line = %s' % single_doc)
        for n, counter in ng_ith_doc.items(): # foreach length and counter
            if not ng_total.has_key(n): ng_total[n] = {}
            ng_total[n].update(counter)
        return
    def get_doc_freq(ng): # <- ngPerDoc 
        for i, entry in ngPerDoc.items(): 
            length = len(ng)
            counter = entry[length]
            if ng in counter: 
                n_match += 1
        return n_match/(len(ngPerDoc)+0.0)
    def consolidate_ngrams(l_counter):
        for n, counter in l_counter.items(): # foreach length and counter
            counts = {}
            for ng, cnt in counter.items(): 
                ng_sorted = sorted(ng)
                if not counts.has_key(ng_sorted): counts[ng_sorted] = cnt 
                counts[ng_sorted] += cnt
            l_counter[n] = collections.Counter(counts)
        return

    import seqAlgo  # count n-grams
    from itertools import chain  # faster than flatten lambda (nested list comprehension)
    import collections
    ...
    save_motif = kargs.get('save_', True); load_motif = kargs.get('load_', False) 
    
    # meta data 
    label = mtype = kargs.get('label', None)

    df = DataFrame(); # length|ngram|n_uniq|count|df
    if load_motif: df = load_ngram()

    ngTopN = {}  # result set
    if df is None or df.empty: # None or empty
        if min_length < 1: min_length = 1
        if max_length < min_length: max_length = min_length

        ngPerDoc = {}  # doc -> (ngram, count)
        for i, doc in enumerate(D): 
            nT = len(doc)
            single_doc = [doc, ]
            ngPerDoc[i] = count_ngrams(single_doc, min_length=1, max_length=min(max_length, nT))  # ngPerDoc[i]: n -> (ng, count)
            # for n, ng_cnts in ngrams.items(): 
            #     ngPerDoc[i] = ng_cnts

        if consolidate_ngrams: 
            pass

        # ngrams = seqAlgo.count_ngrams2(D, min_length=1, max_length=max_length, partial_order=partial_order) 
        
        ### consolidate per-document statistics
        ngTotal = {l: collections.Counter() for l in range(min_length, max_length+1)}  # ngram counts wrt all documents
        for i, doc in enumerate(D): 
            update(ngPerDoc[i], ngTotal)

        ### keep only topn
        for length in range(1, max_length+1):  # +1: inclusive
            counter = ngTotal[length]
            ngTopN[length] = topn_counts = counter.most_common(topn)  # set to None to get all  {(ng, count)}

            # [test]
            assert isinstance(topn_counts[0][0], tuple), "Invalid ngram repr: %s" % str(topn_counts[0][0])
            print("  + top-%d %d-gram frequencies(cid=%s | ct=%s, ot=%s):\n%s\n" % \
                (topn, ng, label, ctype, otype, str(topn_counts[:10])))  #  [('c', 2), ('b', 2)]
            
        apply_groupby_sort, filter_low_freq = True, True
        # save both csv and pkl

        ### 1. csv
        header = ['length', 'ngram', 'n_uniq', 'count', 'df']  # 'ratio': ccnt/gcnt
        adict = {h:[] for h in header}
        for length, ng_counter in ngTopN.items(): 
            for ng, cnt in ng_counter.items(): 
                adict['length'].append(length)
                adict['ngram'].append(ng)
                adict['n_uniq'].append(len(set(ng)))
                adict['count'].append(cnt)
                adict['df'].append(get_doc_freq(ng)) 
        ...
        if apply_groupby_sort: 
            df = df.sort_values(['length', 'count', ], ascending=[False, False, ])  # descending order
