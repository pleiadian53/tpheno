...
#   Module 
#   ------ 
#   tpheno.seqmaker.seqUtils
#   
#   Local utility functions pertinent to the seqmaker module
#
#   Related 
#   -------
...
def sample_class(ts, n_sample=1000, **kargs):
    ...
    if kargs.has_key('n_samples'): n_sample=kargs['n_samples']  # compatibility

    y = ts[TSet.target_field].values
    labels = list(set(y))
    n_labels = len(labels)
    # nrows = ts.shape[0]
    n_subsets = utils.divide_interval(n_sample, n_parts=n_labels) # 10 => [3, 3, 4]
    ...
    for l, n in pdict.items(): 
        ...
        # tse = ts.loc[ts[TSet.target_field]==l]
        # if tse.shape[0] < n: 
        #     tsx.append(tse.sample(n=n, replace=True)) 
        tsx.append(ts.loc[ts[TSet.target_field]==l].sample(n=n, replace=replace))
