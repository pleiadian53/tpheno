...
class lcsHandler(seqConfig.lcsHandler): 
    @staticmethod
    def lcs_file_id(**kargs): # params: {cohort, lcs_type, lcs_policy, seq_ptype, slice_policy, consolidate_lcs, length}
        ...
        adict = {}
        adict['cohort'] = kargs.get('cohort', lcsHandler.cohort)  # this is a mandatory arg in makeLCSTSet()
        adict['lcs_type'] = ltype = kargs.get('lcs_type', lcsHandler.lcs_type)  # global: Common LCSs defined in global scope (as opposed to cluster or local scopes)
        adict['lcs_policy'] = kargs.get('lcs_policy', lcsHandler.lcs_policy) # definition of common LCSs

        # use {seq_ptype, slice_policy, length,} as secondary id
        adict['suffix'] = adict['meta'] = suffix = lcsHandler.secondary_id(**kargs)
        return adict

    @staticmethod
    def lcs_stats_id(**kargs):  # relative frequency
        # same lcs_file_id() except for secondary_id
        pass 

    @staticmethod
    def secondary_id(**kargs): # attach extra info in suffix
        ctype = kargs.get('seq_ptype', lcsHandler.ctype)
        
        suffix = ctype 
        if lcsHandler.do_slice(): suffix = '%s-%s' % (suffix, kargs['slice_policy'])
        ...
        meta = kargs.get('meta', lcsHandler.meta)
        if meta is not None: suffix = '%s-U%s' % (suffix, meta)  # U: user-provided e.g. pattern_type: enriched vs rare
    ...
    def do_slice(**kargs): # only for file ID, see secondary_id()  
        slicePolicy = kargs.get('slice_policy', lcsHandler.slice_policy)
        if slicePolicy.startswith(('noop', 'reg', 'compl', )):  # effective operations 
            return False 
        return True

    @staticmethod  
    def load_lcs(**kargs): # [note] for now, only used in makeLCSTSet()
        """
        ...
        1. example path to the LCS-pattern file
           <prefix>/tpheno/seqmaker/data/CKD/pathway/lcs_local-df-regular-iso-LControl-Uenriched.csv
                ...
                <secondary id> regular-iso-LControl-Uenriched    //seq_ptype (regular) is implicit here, a class attribute not a function argument (**kargs)
        ...
        # params: cohort, lcs_type, lcs_policy, seq_ptype, label, meta

        ctype0 = lcsHandler.ctype

        ctype = kargs.get('ctype', None)
        if ctype is not None: lcsHandler.ctype = ctype  # {'regular', 'med', 'diag'}; this is used implicitly in secondary_id

        adict = lcsHandler.lcs_file_id(**kargs) #
        cohort = adict['cohort']
        ltype, lcs_select_policy = adict['lcs_type'], adict['lcs_policy']
        suffix = adict['meta'] 
        print('\n(load_lcs) pathway params > ltype: %s, lcs_policy: %s, suffix/meta: %s' % (ltype, lcs_select_policy, suffix))
        df = Pathway.load(cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix, dir_type='pathway')

        lcsHandler.ctype = ctype0  # restore ctype so that this operation does not affect the global setting
        if df is None: 
            return DataFrame() # dummy dataframe
        return df

    @staticmethod
    def save_lcs(df, **kargs): 
        if df is None or df.empty:
            print('Warning: Null dataframe! Aborting ...')
            return 
    ...
    @staticmethod
    def save_relative_stats(df, **kargs):
        pass 

    @staticmethod
    def load_relative_stats(**kargs): 
        pass
    ...
    @staticmethod
    def save_sparse_tset(X, y, fset=[], policy_fs='sorted', meta=None): 
        return TSet.save_tset(X, y, fset, policy_fs=policy_fs, meta=meta)
    @staticmethod
    def save_tset(X, y, fset=[], policy_fs='sorted', meta=None):
        from tset import TSet
        # file ID: cohort, d2v_method, seq_ptype, index, suffix
        # directory: cohort, dir_type/'combined'

        if meta is None: meta = lcsHandler.meta
        fileID = meta
        # if policy_fs.startswith('so'): # sorted, tfidf, tf, ... 
        #     fileID = 'sorted' if meta is None else '%s-sorted' % lcsHandler.meta

        nF = len(fset)
        if nF > 0: assert X.shape[1] == nF, "save_tset> ncol(X):%d <> size(fset):%d" % (X.shape[1], nF)
            
        if not isinstance(y, np.ndarray): y = np.array(y)
        TSet.saveSparseLCSFeatureTSet(X, y=y, cohort=lcsHandler.cohort, d2v_method=lcsHandler.d2v_method, 
            seq_ptype=lcsHandler.ctype, suffix=fileID, header=fset)   # saveSparseLCSFeatureTSet() -> saveSparse()
    ...
    @staticmethod
    def load_sparse_tset(policy_fs='sorted', meta=None): 
        return lcsHandler.load_tset(policy_fs=policy_fs, meta=meta)
    @staticmethod
    def load_tset(policy_fs='sorted', meta=None):
        """
        Load the (sparse) training set data in which features are LCS patterns. 

        Input
        -----
        policy_fs: policy for feature set selection and arrangement. 
                  'sorted' by default, for which features are arranged in alphanumeric order. 
        """
        ...
        # if policyFeatureRank.startswith('so'): # sorted, tfidf, tf, ... 
        #     fileID = 'sorted' if meta is None else '%s-sorted' % lcsHandler.meta

        # [note] seq_ptype is less meaningful here because the LCS features can very well have come in any of the following forms: 
        #        1. regular: LCS comprises a mixture of med and diag 
        #        2. med: LCS comprises only med 
        #        3. diag: LCS comprises only diag
        #        lcsHandler.ctype is regular by default
        print('(load_tset) Params: cohort=%s, d2v=%s, ctype=%s, suffix=%s' % (lcsHandler.cohort, lcsHandler.d2v_method, lcsHandler.ctype, fileID))
        X, y, fset = TSet.loadSparseLCSFeatureTSet(cohort=lcsHandler.cohort, d2v_method=lcsHandler.d2v_method, 
                          seq_ptype=lcsHandler.ctype, suffix=fileID)
        if len(fset) == 0: 
            print('(load_tset) Warning: Missing feature set.')
        return (X, y, fset)
...
def analyzeMCS(D, T, lcs_set, **kargs):
    ...
    """
    def load_stats(verbose=True):
        temp_dir = kargs.get('tempdir', os.path.join(os.getcwd(), 'temp'))
        fpath = os.path.join(temp_dir, kargs.get('outputfile', 'analyzeMCS.pkl'))

        ret = pickle.load(open(fpath, 'rb')) if os.path.exists(fpath) else {}
        if verbose: 
            if ret.has_key('index') and len(ret['index']) > 0: 
                msg = '(load_stats) Loading precomputing result from:\n%s\n' % fpath
                msg += '... found %d patterns\n' % len(ret['index'])  # [hardcode]
                stats = get_stats(ret, entry='index')
                if stats: 
                    msg += '... how many LCS patterns does an MCS document generally have?\n'
                    msg += '...... min: %d, max: %d | mean: %d, median: %d\n' % \
                        (stats['min'], stats['max'], stats['mean'], stats['median'])
                print msg
            else: 
                print('(load_stats) no data found at the path:\n%s\n' % fpath)
        return ret 
    def save_stats(ret, verbose=True): 
        temp_dir = kargs.get('tempdir', os.path.join(os.getcwd(), 'temp'))
        if not os.path.exists(temp_dir): os.mkdir(temp_dir) 

        fpath = os.path.join(temp_dir, kargs.get('outputfile', 'analyzeMCS.pkl'))
        pickle.dump(ret, open(fpath, 'wb'))
        if verbose: 
            print('(save_stats) Saving precomputing result to:\n%s\n' % fpath)
        return
    def get_stats(hashtb, entry='index'):
        values = (len(alist) for alist in hashtb[entry])  # e.g. doc_id -> {lcs}
        stats = {}
        try:  
            stats['max'], stats['min'] = max(values), min(values)
            stats['median'], stats['mean'] = np.median(values), np.mean(values)
        except: 
            print('(get_stats) empty map? values=%s' % list(values))
        return stats

    # [output] load pre-computed results (necessary, this operation being usually very time-consuming)
    tLoad = kargs.get('load_result', False)
    if tLoad:  
        ret = load_stats(verbose=True)
        if len(ret.get('index', [])) > 0: 
            return ret
    ...
    # [output] save pre-computed results
    tSave = kargs.get('save_result', False)
    if tSave:  # [note] operations such as analyze_strata() may need this result
        save_stats(ret)
...
def analyzeLCS2(D, T, lcs_set, **kargs):
    ...
    **kargs
    ...
    def eval_lcs_stats(): # [params] lcsmap, lcsmapInv, matched_docIDs
        ...
        r = nM/(nD+0.0)
        print('\n(eval_lcs_stats) number of docs found match: %d, ratio: %f' % (nM, r))

        # number of documents with multiple LCS labels? 
        n_multimatch = n_singlematch = n_nomatch = 0
        # tidx = random.sample(lcsmapInv.keys(), min(len(lcsmapInv), 10))
        for i in range(nD): 
            if i in lcsmapInv:  # lcsmapInv: doc_id -> {lcs_i}
        ...
        print('    + number of documents %d | n_multilabel:%d, n_single: %d, n_nomatch: %d' % (nD, n_multimatch, n_singlematch, n_nomatch))

        # most popular LCSs? 
        print('(eval_lcs_stats) validate most popular LCSs ...')
        ...
        # randomized lcs (just to verify their contents)
        print('(eval_lcs_stats) validate randomized LCSs ...')
        n_test = min(len(lcsmapInv), 50)
        idx = random.sample(lcsmapInv.keys(), n_test)
        randomLCSs = set()
        for i in idx: 
            randomLCSs.update(lcsmapInv[i])  # one doc can reference multiple LCSs
        for lcs in randomLCSs: 
            print('    + LCS: %s found %d matches ...' % (lcs, len(lcsmap[lcs])) )
    ...
    def make_timeseries_repr(i, lcs_seq, index_set): # lcsTimeMap, D, T
        ...
        return lcsTimeMap
    def verify_lcs_candidates(n=10): 
        N = len(lcs_set)
        print('(verify_lcs_candidates) example input LCS patterns (n=%d): ' % n)
        for i, lcs in enumerate(random.sample(lcs_set, min(n, N))): 
            print('  + [%d] %s' % (i+1, lcs))
    ...
    print('analyzeLCS2> Find MCS documents containing a given LCS (n=%d); do this for each LCS ...' % nL)
    # verify_lcs_candidates()
    ...
    nD_has_labels = 0
    tMakeColorTime = kargs.get('make_color_time', False)
    for i, doc in enumerate(D): # foreach doc, find its LCS labels
        ...
        if i > 0 and i % 1000 == 0: print('... completing %f%% (%d out of %d docs)' % (i/(nD+0.0), i, nD))
    
    # when comparing patterns in different strata, it's possible that the input LCSs are not found at all in the other strata
    if kargs.get('check_df_min', True):  
        assert nD_has_labels >= 2, "Only %d documents have matching LCSs (nD=%d, nLCS=%d)" % (nD_has_labels, nD, nL)
    if tMakeColorTime: assert len(lcsColorMap) > 0 and len(lcsTimeMap) > 0
    eval_lcs_stats() # [params] lcsmap, lcsmapInv, matched_docIDs
...
def analyzeLCS3(D, T, lcs_set, **kargs):
    ...
    **kargs
    ...
    nD = len(D)
    nL = nLCS = len(lcs_set)  # plus 'No_Match' LCS label
    # print('analyzeLCS3> Find MCS documents containing a given LCS (n=%d); do this for each LCS ...' % nL)
    ...
    # when comparing patterns in different strata, it's possible that the input LCSs are not found at all in the other strata
    if kargs.get('check_df_min', True): 
        msg = "Only %d documents have matching LCSs (nD=%d, nLCS=%d)" % (nD_has_labels, nD, nL) 
        # assert nD_has_labels >= 2, msg
        if nD_has_labels < 2: print msg
...
def analyzeLCS(D, lcs_set, **kargs): 
    ...
    print('analyzeLCS> Find MCS documents containing a given LCS (n=%d); do this for each LCS ...' % nL)
...
# [helper] deriveLCS, deriveLCS2
def filter_lcs_by_uniq(lcsmap, topn=20, min_df=50): # params: lcsmap
...
# [helper] deriveLCS, deriveLCS2 
def filter_lcs_by_docfreq(lcsmap, topn=20, min_df=2, reverse=True): # params: lcsmap, minNDoc
    # params: set reverse to True to find top N LCS 
    #         set to False to find more rare N LCS
    ...
    if nl > topn: 
        ...
        ranked = sorted([(c, len(lcsmap[c])) for c in candidates], key=lambda x: x[1], reverse=reverse)
        extremeLCS, extremeRankScore = ranked[0][0], ranked[0][1]
        if reverse: # high df to low df 
            print("  + maximum DF: %d, enriched LCS=\n%s\n" % (extremeRankScore, extremeLCS))
        else: # low df to high df
            print("  + minimum DF: %d, rare LCS=\n%s\n" % (extremeRankScore, extremeLCS))
...
def deriveLCS2(D, **kargs):
    ...
    def filter_by_anchor(): # [params] lcsmap: lcs -> docIDs
        ...
        return lcsmap2
    def filter_by_policy(lcsmap, topn=100, min_df=2, reverse=True): # params: lcsmap
        # use only filters above
        N0 = len(lcsmap)  # direct access of lcsmap won't work with nested calls below
        policy = kargs.get('lcs_policy', 'df') 
        lcsmap_minus = None
        if policy.startswith('u'):  # uniqnuess
            lcsmap_minus = filter_lcs_by_uniq(lcsmap, topn=topn, min_df=min_df) # select top n LCSs, all of which occurr in at least m documents 
        elif policy.startswith('d'):  # df, document frequency
            lcsmap_minus = filter_lcs_by_docfreq(lcsmap, topn=topn, min_df=min_df, reverse=reverse)
        elif policy == 'noop': 
            lcsmap_minus = lcsmap
        else: 
            raise NotImplementedError, "unrecognized LCS filter strategy: %s" % policy
        print("deriveLCS2.filter_by_policy> lcsmap size %d -> %d | policy=%s, topn=%d, min_df=%d" % \
            (N0, len(lcsmap_minus), policy, topn, min_df))
        return lcsmap_minus
    def analyze(): 
        # [note] design 
        T = kargs.get('T', [])
    ...
    df = lcsHandler.load_lcs(**kargs) if tLoadLCS else None  # try loading first | params: overwrite_lcs, 
    tNewLCS = False

    minLCSDocFreq = kargs.get('min_ndocs', 2)     # min(len(D)/10, 10)
    if df is None:  # df: ['length', 'lcs', 'n_uniq', 'count', 'df', ]
        ...
        df = deriveLCS(D=D, # label=process_label(label) 
                        ...
                        min_ndocs=minLCSDocFreq,  # depends on the size of D
                        ...
                        incremental_update=True, # if True, do load_lcs() first and add new LCS patterns to it
                        return_type=kargs.get('return_type', 'enriched'), # values: {'enriched', 'rare', }
...
def deriveLCS(D, **kargs):
    ...
    def normalize_docs(): # convert input (D) into a canonical form
        ...
        return DCluster
    def make_pairs(D_subset, cid=0, policy='random', n_pairs=100000): # [params] maxNPairs, policy={'longest_first', 'random'}
        # select pairs of documents from which to derive LCSs 
        n = len(D_subset)
        # pairwise lcs   
        n_total_pairs_est = n_total_pairs = int((n*(n-1))/2.)  # approx.

        # [note] this will fail when size(D_subset) becomes too large
        ...
        # n_total_pairs = sum(1 for x in pairs)
        # print(' + n_total_pairs: %d =?= est %d' % (n_total_pairs, n_total_pairs_est))
        if policy.startswith('rand'): # randomized
            print('(make_pairs) n_pairs: %d vs n_total_pairs: %d' % (n_pairs, n_total_pairs))
            pairs = sampling.random_items2(pairs, min(n_pairs, n_total_pairs)) # random.sample(pairs, min(maxNPairs, npt)) 
            ...
            n = (math.sqrt(8 * n_pairs)+1)/2  # number of documents needed to make max pairs
            n = int(math.ceil(n)) # overesimate a bit 
            pairs = (pair for pair in  itertools.combinations([i for i, _ in plist[:n]], 2))
        ...
        # n_pairs_eff = sum(1 for x in pairs)  # this exahust the generator and disable the use of pairs as an iterator later
        cid = normalize_cluster_id(cid)
        # print('  + [cluster %s] choose %d out of %d possible document pairs' % (cid, n_pairs_eff, n_total_pairs))
        return pairs
    ...
    def filter_by_policy(lcsmap, topn=100, min_df=2, reverse=True): # params: lcsmap
        ...
        if policy.startswith('u'):  # uniqnuess
            ...
            raise NotImplementedError, "unrecognized LCS filter strategy: %s" % policy
        print("filter_by_policy> lcsmap size %d -> %d | policy=%s, topn=%d, min_df=%d" % (N0, len(lcsmap_minus), policy, topn, min_df))
        return lcsmap_minus
    ...
    def test_lcs_consistency(): # lcsmapEst vs lcsmap, lcsmapEst should have more counts
        ...
        for s, dx in lcsmapEst.items(): 
            ...
            if n >= nT: break
        return  
    def test_min_docs(lcsmap): # lcsmap (after filter_lcs operation), minNDoc
    ...
    def process_label(l): 
        # remove spaces e.g. CKD Stage 4
        return ''.join(str(e) for e in l.split())
    def eff_docs(lcsmap): # params: lcsmap
    ...
    def show_params(cid='n/a'): 
        ...
        print('  >>>              label: %s <<<' % kargs.get('label', 'n/a'))
    ...
    topn_lcs = kargs.get('topn_lcs', 100)   # but predominent will be shorter ones
    raren_lcs = kargs.get('raren_lcs', 100)
    minNDoc = minDocFreq = kargs.get('min_ndocs', 2)  # candidate LCSs must appear in >= min_ndocs documents
    ...
    for cid, documents in DCluster.items(): # unless specified otherwise, assume that all documents (D) are one big cluster
        ...
        # [note] keep pairs as a generator to conserve memory
        pairs = make_pairs(D_subset=documents, cid=cid, policy=pairing_policy, n_pairs=maxNPairs) # [params] policy='longest_first'
        
        pairs = list(pairs)  # this may use a lot of memoery if maxNPairs is huge
        n_total_pairs = len(pairs)

        # sum(1 for x in pairs) # if pairs is a generator, this exahusts it! 
        # assert n_total_pairs >  maxNPairs/2, "n_total_pairs: %d not much lg than maxNPairs: %d" % (n_total_pairs, maxNPairs) 
        print('  + iterating over %d pairs' % n_total_pairs)
        show_params(cid=cid)

        n_pairs = n_iter = n_void = 0
        ...
        verify_lcs(cid)  # [test] 
    ### end foreach (cid, doc)
    print('  + looped through %d pairs > %d eff pairs (=?= desired: %d | %d void' % (n_iter, n_pairs, maxNPairs, n_void))
    ...
    # policy: find both enriched and rare LCS patterns
    # [note] if label is not present, then it is a global LCS candidate set, in which case there's no distinction in pattern type 
    #        => setting: [(None, False), ] # use globally rare LCS patterns

    label_id = kargs.get('label', None)
    settings = [(None, False),]; retType = None
    if label_id is not None: # finding locally enriched/rare LCS using either the global candidates or local candiates
        # pattern_type: {'enriched', 'rare'} is more of a local property
        settings = [('enriched', True), ('rare', False), ]  # find top N most freq LCS, set reverse to True; o.w. set to False
        retType = kargs.get('return_type', 'enriched')
    # return types: {'enriched', 'rare', None}
    assert retType in dict(settings), "Unknown return type: %s" % retType

    header = ['length', 'lcs', 'n_uniq', 'count', 'df', ] # Pathway.header_global_lcs # ['length', 'lcs', 'count', 'n_uniq', 'df', ] 
    df_ret = DataFrame(columns=header)  # dummy dataframe 
    
    lcsmap_minus = {}
    for setting in settings: 
        meta, reverse = setting[0], setting[1]  # meta: file descriptor
        kargs['meta'] = meta  

        lcsmap_minus = filter_by_policy(lcsmap, topn=topn_lcs, min_df=minNDoc, reverse=reverse)  # option: lcs_policy
        test_min_docs(lcsmap_minus) # params: nL0, nL, nLc, lcsmap

        # compute statistics and save data
        adict = {h:[] for h in header}

        nDEff = len(eff_docs(lcsmap_minus))
        print('deriveLCS> effective number of documents where candidate LCSs are present: %d (out of %d => r: %f)' % (nDEff, nDoc, nDEff/(nDoc+0.0)))

        df0 = lcsHandler.load_lcs(**kargs) if kargs.get('incremental_update', True) else None # incremental_update: always load if already existed
        lcsCnt = {}
        ...
        lcsHandler.save_lcs(df, **kargs) # lcs_type, lcs_policy

        # return value: enriched or rare? 
        if retType == meta: df_ret = df

    assert not df_ret.empty
    return df_ret # (df, lcsmap) # LCS -> document IDs  (this is needed to label the document set)
...
def docToLCS(cohort, **kargs): 
    ...
    def sample_docs(D, L, T, n, sort_index=True, random_state=53):
        ...
        print('  + subsampling: select only %d docs.' % len(D))
        return (D, L, T)
    def process_docs(): # params: cohort
        ### load + transfomr + (ensure that labeled_seq exists)
        
        src_dir = kargs.get('inputdir', None)
        if src_dir is None: src_dir =  get_global_cohort_dir()

        ctype = lcsHandler.ctype # kargs.get('seq_ptype', 'regular')
        ...
        tSimplified = kargs.get('simplify_code', lcsHandler.is_simplified)
        D, L, T = processDocuments(cohort=lcsHandler.cohort, seq_ptype=ctype, 
                    inputdir=src_dir, 
                    ...
                    # remove code prefix 
                    remove_cond_prefix=True, 
                    remove_med_prefix=False, 
    ...
    D, L, T = process_docs() # cohort, ctype, *inputdir (None to use default), *max_n_docs
    ...
    df = deriveLCS2(D=D, 
                    ...
                    lcs_type=kargs.get('lcs_type', 'global'), 
                    lcs_policy=kargs.get('lcs_policy', 'df'), # lcs filtering policy

                    # update policy: compute a brand new set or add to the existing? 
                    return_type=kargs.get('return_type', None), # values: {'enriched', 'rare', }
...
def makeLCSFeatureTSet(cohort, **kargs):
    ...
    def save_tset(X, fset=None, overwrite=True): # <- nD, mark training data 
        ...
        # load + suffix <- 'Flcs'
        ts = TSet.loadTSet(cohort=cohort, d2v_method=d2v_method, seq_ptype=seq_ptype) # use default 'Flcs' keyword
...
def makeLCSFeatureTSet2(cohort, **kargs): 
    ...
    * Persistent 
    ...
    def save_tset(X, y, fset=[], policy_fs='sorted', meta=None):
        ...
        # PS2: load method
        # X, y, fset = TSet.loadSparseLCSFeatureTSet(cohort, **kargs)
        return lcsHandler.save_tset(X, y, fset=fset, policy_fs=policy_fs, meta=meta)
    def load_tset(policy_fs='sorted', meta=None):
        return lcsHandler.load_tset(policy_fs=policy_fs, meta=meta)
        # return (X, y, fset)
    ...
    if len(lcsCandidates) == 0: 
        ...
        df = lcsHandler.load_lcs(lcs_type='global', lcs_policy='df', meta=None)  # load the global LCS training set
    ...
    if kargs.get('save_', True): 
        save_tset(Xs, L, fset=lcsFSet, meta=kargs.get('pattern_type', 'enriched'))
...
def chooseLCSFeatureSet(topn, **kargs): 
    ...
    def rank_fset(n_features, docIds=None, inputdir=None, content_sep=','): 
        ...
        for lcs, gtf in gTermFreq.items():  # gtf: global term frequency
...
def loadLCSFeatureTSet(**kargs): # backward compatibility
...
def loadTSet(**kargs):
    ...
    def save_tset(X, y, fset=[], policy_fs='sorted', meta=None):
        ...
        # X, y, fset = TSet.loadSparseLCSFeatureTSet(cohort, **kargs)
        return lcsHandler.save_tset(X, y, fset=fset, policy_fs=policy_fs, meta=meta) 
    def load_tset(policy_fs='sorted', meta=None):  # policy_fs is not used now, assuming that feature order is always alphanumerically sorted
        if meta is None: meta = lcsHandler.meta
        fileID = meta  # lcsHandler.meta
    ...
    ### load 
    X, y, fset = load_tset(meta=kargs.get('pattern_type', 'rare'))
    profile(X, y)  # initial 
    ...
    # a. select subgroups, relabeling
    if X is not None and y is not None:
        print('loadTSet> Customizing training set (a. select only particular subgroups b. merge labels, ...)') 
        X, y = customize_tset(X, y)

    n_classes0 = len(np.unique(y))

    # b. drop explicit control data in multiclass
    if kargs.get('drop_ctrl', False): # drop control group (e.g. label: 'Others')
        ...
        X, y = remove_classes(X, y, labels=['Ctrl', ])
...
def makeLCSFeatureTSet2a(cohort, **kargs):  # [note] maybe do away with the explicit arg: cohort
    ...
    def eval_feature_value(fset, inputdir=None, content_sep=','): 
        ...
        name = 'lcsmap'
        gDocFreq = {}
        for lcs, idx in initvarcsv(name=name).items():  # lcs -> [d1, d2, ...], where di: document IDs
        ...
        gTermFreq = {}
        for i, lcsx in initvarcsv(name=name).items(): # lcsx: a list of (lcs, tf)
            gTermFreq[i] = dict(lcsx)  # initvarcsv(name='lcsmapInvFreq')  # id -> {(lcs, tf)}
        ...
        for i, doc in enumerate(D):
            ...
            col_idx = range(nF) # [lcsToFPos[lcs] for lcs, _ in gTermFreq[i] if lcs in lcsToFPos]  #  docID -> {(lcs, freq)} -> {(pos, freq)}
                
            # values = [tf or _, tf in gTermFreq[i]] # term frequency only
            values = []
            # for lcs, tf in gTermFreq[i]: 
            for i, lcs in enumerate(fset): 
                tf = gTermFreq[i].get(lcs, 0) if gTermFreq.has_key(i) else 0
                gdf = gDocFreq.get(lcs, 0) 
                w = (1+nD)/(1+gdf+0.0)
    ...
    assert Xs.shape[0] == len(L)
    if kargs.get('save_', True): 
        save_tset(Xs, L, fset=lcsFSet)
    
    return (Xs, L)

def makeLCSFeatureTSet3(cohort, **kargs): 
    """
    Similar to makeLCSFeatureTSet2a but select least frequent LCS features that 
    occur in at least N documents (N=2 by default)

    Memo
    ----

    1. return value from analyzeMCS(): 

       A dictionary with key: index, frequency, inverted_index, color, time 
    ...
    """
    def stratify_docs(lmap={}): # params: cohort
        ### load + transfomr + (ensure that labeled_seq exists)
        ...
        if ifile is not None: 
            ...
            ifiles = [ifile, ]
        assert os.path.exists(src_dir), "Invaild MCS source dir:\n%s\n" % src_dir
        if not ifiles: ifiles = ['condition_drug_labeled_seq-CKD.csv', ]
        ...
        if not lmap: lmap = seqparams.System.relabel() # policy_relabel() 
        stratified = stratifyDocuments(cohort=cohort, seq_ptype=ctype,
                    predicate=kargs.get('predicate', None), 
                    simplify_code=kargs.get('simplify_code', tSimplified), 
        ...
        return stratified
    def process_docs(ctype='regular'): 
        ### load + transfomr + (ensure that labeled_seq exists)
        src_dir = kargs.get('inputdir', None)
        if src_dir is None: src_dir =  get_global_cohort_dir()
        ...
        # Dl = labelize(D, label_type='doc', class_labels=L)  # use class labels for document label prefixing
        is_labeled = len(np.unique(L)) > 1
        print('(process_docs) nD: %d | cohort=%s, ctype=%s, labeled? %s, simplified? %s' % (len(D), lcsHandler.cohort, ctype, is_labeled, tSimplified))
        return (D, L, T)
    ...
    def stratum_stats(units): # <- units
        ...
        return
    def eval_feature_value(D, L, fset, lcs_result, inputdir=None, content_sep=','): 
        # Same as initvar but load lcs map contents from .csv files 
        # cohort = kargs.get('cohort', lcsHandler.cohort)  # 'CKD'
        ...
        # first check lcs to docIDs ...
        # name = 'lcsmap'
        gDocFreq = {} # in incremental mode: initvarcsv(name='lcsmap') 
        for lcs, idx in lcs_result['index'].items(): # lcs_result['index']: lcs -> docIDs
            gDocFreq[lcs] = len(idx)

        # ... then check docID -> {(lcs, freq)}
        gTermFreq = {}
        for i, lcsx in lcs_result['frequency'].items(): # lcsx: a list of (lcs, tf)
            gTermFreq[i] = dict(lcsx)  # initvarcsv(name='lcsmapInvFreq')  # id -> {(lcs, tf)}
        ...
        for i, doc in enumerate(D):
            ...
            col_idx = range(nF) # [lcsToFPos[lcs] for lcs, _ in gTermFreq[i]]  #  docID -> {(lcs, freq)} -> {(pos, freq)}
                
            # values = [tf or _, tf in gTermFreq[i]] # term frequency only
            values = []
            
            # find feature values for the ith document 
            for j, lcs in enumerate(fset): # foreach feature; fill the feature value according to input feature order
                # tf = gTermFreq[i].get(lcs, 0) if gTermFreq.has_key(i) else 0  # but gTermFreq should contain the document ID 
                tf  = gTermFreq[i].get(lcs, 0)  # term frequency within document i 
                gdf = gDocFreq.get(lcs, 0) # global document frequency (gdf: number of docs containing the lcs, not sum of total counts)
                w = (1+nD)/(1+gdf+0.0)
                score = tf * (math.log(w)+1.0)
                values.append(score)  # tf-idf scores

            if True: # len(values) > 0: 
                row.extend([i] * len(col_idx))  # ith document
                col.extend(col_idx)  # jth attribute
                data.extend(values)  # scores
                nDEff += 1 
        ...
        Xs = csr_matrix((data, (row, col)), shape=(nDEff, nF))
        assert Xs.shape[0] == nD and Xs.shape[1] == nF
        
        print('makeLCSFeatureTSet3> Found %d coordinates, %d active values | nD: %d >? nDEff: %d ' % \
            (len(data), Xs.nnz, nD, nDEff))
        gc.collect()
        return (Xs, Ls)
    def save_tset(X, y, fset=[], policy_fs='sorted', meta=None):
        # file ID: cohort, d2v_method, seq_ptype, index, suffix
    ...
    def load_tset(policy_fs='sorted', meta=None):  # policy_fs is not used now, assuming that feature order is always alphanumerically sorted
        ...
        return lcsHandler.load_tset(policy_fs=policy_fs, meta=meta)
        # return (X, y, fset)
    def eval_proportion(L, labels=None):
        if labels is None: labels = np.unique(L) 
        lc = collections.Counter(L)
        N = len(L) # sum(c for c in lc.values())
        fractions = {label: lc[label]/(N+0.0) for label in labels}  # label to fraction
        return fractions  
    def clean_up(): 
        temp_path = os.path.join(tempDir, tempFile)
        if os.path.exists(temp_path):  
            os.remove(temp_path)
        else: 
            print('(clean_up) temporary file never existed? %s' % temp_path)
        return     
    ...
    policyVars = kargs.get('lcs_var_policy', 'local')
    print('makeLCSFeatureTSet3> feature set type: %s ...' % policyVars)
    n_features = nF0 = kargs.get('topn', 10000)  # topn? 
    patternType = kargs.get('pattern_type', 'rare')
    lcsPolicy = kargs.get('lcs_policy', lcsHandler.lcs_policy)

    # [note] set ctype to regular to load the entire document
    D, L, T = process_docs(ctype='regular')  # need to know the labels in local feature mode
    labels = np.unique(L)

    # feature types: a subset of ['med', 'diag', 'regular', ]
    #    typical values: 
    #        ['med', 'diag']
    #        ['regular', ]
    ctypes = kargs.get('feature_types', ['regular', ])  # use medication LCSs (med), diag LCSs (diag), or mixture (regular) as features? 
    isMulticlass = True if len(L) > 0 and len(labels) > 1 else False

    dfG = []
    if policyVars.startswith('gl'):  # use global feature set 
        lcsType = 'global'

        # ctypes: ['med', 'diag', ], ['regular', ]
        for ctype in ctypes:
            df = lcsHandler.load_lcs(lcs_type=lcsType, lcs_policy=lcsPolicy, meta=patternType, ctype=ctype)  # local global LCS candidates 
            dfG.append(df)

        # lcsCandidates = dfG.sample(n=nF)['lcs'] # dfG['lcs'].values
    else: # local 
        lcsType = 'local'
        nL = len(L)

        # use regular patterns? (i.e. mixing diag and med segments) or use med and diag separately? 

        # [note] set lmap to {} to use default label mapping
        label_map = {}
        class_labels = seqparams.System.convert_labels(labels=labels, lmap=label_map, unique=True) 
        print('(test) mapped, unique class labels: %s' % class_labels)

        # [note] if labels were unmapped, then do: fractions = eval_proportion(L, labels=labels) 
        L_mapped = seqparams.System.convert_labels(labels=L, lmap=label_map)
        fractions = eval_proportion(L=L_mapped, labels=class_labels)
   
        # use regular patterns? (i.e. mixing diag and med segments) or use med and diag separately?
        # ctypes: ['med', 'diag', ], ['regular', ]
        n_offset = 20
        for ctype in ctypes: 
            for label in class_labels:  # [attention] need to use mapped labels 
                df = lcsHandler.load_lcs(label=lcsHandler.process_label(label), 
                        lcs_type=lcsType, lcs_policy=lcsPolicy, meta=patternType, ctype=ctype) # lcs_type=lcsType, lcs_policy=lcsPolicy
            
                nS = min( int(fractions.get(label) * n_features)+n_offset , df.shape[0])
                print('... ctype: %s > loading lcs for label: %s | n(%s)=%d' % (ctype, label, label, nS)) # e.g. lcs_local-df-regular-iso-LCKDG1-control-Urare.csv

                df = df.sample(n=nS)   # sampling according to class proportion      
                dfG.append(df)
            ## end foreach (mapped) class label
        ## end foreach feature type
    
    ### end feature acquisition
    dfG = pd.concat(dfG, ignore_index=True)
    
    if dfG is None: print('makeLCSFeatureTSet3> Warning: No existing feature file found!'); dfG = DataFrame() # dummy
    
    nF = min(n_features, dfG.shape[0]) 
    print('... desired %d vars, sampled dataframe dfG has %d rows | took min => %d' % (n_features, dfG.shape[0], nF))

    # use a smaller sample size to speed up
    # [note] the "count" statistics may not be reliable due to subsampling
    lcsCandidates = dfG.sample(n=nF)['lcs'] # dfG['lcs'].values
    nF = len(lcsCandidates)
    print('... selected %d (from %d) LCS canidates ...' % (nF, dfG.shape[0]))  # [note] got 43406 as of 07.18

    ### global analysis 
    # ctype = kargs.get('seq_ptype', lcsHandler.ctype)  # kargs.get('seq_ptype', 'regular')
    feature_id = '-'.join(ctypes)
    tempDir = os.path.join(os.getcwd(), 'temp')
    tempFile = 'makeLCSFeatureTSet3.analyzeMCS-%s.pkl' % feature_id
    ret = analyzeMCS(D, T, lcs_set=lcsCandidates, make_color_time=False, incremental_mode=False, 
            save_result=True, load_result=True, 

                # caching temporary output that is expensive to compute every time
                tempdir=tempDir, # result caching dir
                outputfile=tempFile)  # index:lcsmap, frequency:lcsmapInvFreq
    assert len(ret['index']) > 0

    # filter, but this may result in smaller feature size (than n_features)
    candidates, disqualified = [], []
    minLCSDocFreq = kargs.get('min_ndocs', 1) 
    nullFSet = []  # null feature set; those that never appear in any document
    for i, lcs in enumerate(lcsCandidates): 
        ids = ret['index'].get(lcs, [])  # lcsmap: lcs -> document IDs 
        ndocs = len(ids)
        fv = (lcs, ndocs)
        if ndocs >= minLCSDocFreq: 
            candidates.append(fv)
        else: 
            # [test]
            if len(disqualified) < 100: 
                print('... disqualifying %s | min_ndocs=%d' % (lcs, minLCSDocFreq))
            disqualified.append(fv)

    nF = len(candidates)

    # feature ordering 
    reverse = False if patternType.startswith('ra') else True  # rare <- (reverse: False -> ascending)
    lcsCandidates = [f for f, cnt in sorted(candidates, key=lambda x: x[1], reverse=reverse)] # least frequent
    print('... finally got %d LCS features <? n_total=%d | pattern type: %s, min df: %d' % (len(lcsCandidates), nF, patternType, minLCSDocFreq))

    # [format] sorted & sparse format by default 
    lcsCandidates = sorted(lcsCandidates)  # keep track of LCS features according to this order

    # policy: use TF-IDF score as the value
    Xs, Ls = eval_feature_value(D, L, fset=lcsCandidates, lcs_result=ret)  # feature values are not scaled
    ...
    assert Xs.shape[0] == len(Ls)
    if kargs.get('save_', True): 
        # save training data + feature set
        save_tset(Xs, Ls, fset=lcsCandidates, meta=patternType) # {'rare', 'enriched'}

    ### post-processing

    # need to remove temp files: 
    # e.g. <prefix>/seqmaker/temp/makeLCSFeatureTSet3.analyzeMCS-regular.pkl
    clean_up()

    # feature filtering
    #    criteria
    #       1. doc freq >= 2 
    #       2. choose rare features (topn)
    
    ### local analysis 

    # stratified = stratify_docs()
    ...
    #     #       1. doc freq >= 2 
    #     #       2. choose rare features (topn)
    #     minLCSDocFreq = 2 # min(len(Di)/10, 2)

    #     # query frequency: number of docs in the other strata in which lcs occurs
    #     # [note] candiate attributes: ['length', 'lcs', 'n_uniq', 'count', 'df', ]
    #     ret = analyzeMCS(Di, Ti, lcs_set=lcsCandidates, make_color_time=False)  # index:lcsmap, frequency:lcsmapInvFreq

    #     candidates = []
    #     for lcs in lcsCandidates: 
    #         ids = ret['index'][lcs]  # lcsmap: lcs -> document IDs 
    #         ndocs = len(ids)
    #         if ndocs >= minLCSDocFreq: 
    #             candidates.append((lcs, len(ids)))
            
    #     rareLCSs = sorted(candidates, key=lambda x: x[1], reverse=False) # least frequent

    #     for lcs in lcs_set: 
    #         ids = ret['index'][lcs]  # lcsmap: lcs -> document IDs 
    #         adict[label].append(len(ids))
...
def makeLCSTSet(cohort, **kargs):
    ...
    d2v_method = kargs.get('d2v_method', seqparams.D2V.d2v_method)
    src_dir = kargs.get('inputdir', seqparams.getCohortGlobalDir(cohort))  # sys_config.read('DataExpRoot')/<cohort>
    ...
    if df is None:  
        ...
        df = deriveLCS(D=D, 
                        ...
                        max_n_pairs=kargs.get('max_n_pairs', 125000), min_ndocs=kargs.get('min_ndocs', 2), 
...
def t_classify0a(**kargs): 
    ...
    def load_tset(fpath=None, index=0):
        ...
        if fpath is not None and (os.path.exists(fpath) and os.path.getsize(fpath) > 0): 
            ...
            ts = TSet.loadTSet(cohort=kargs.get('cohort', 'CKD'), 
                        d2v_method=kargs.get('d2v_method', vector.D2V.d2v_method), 
...
def t_lcs_strata(**kargs):
    return t_lcs_local(**kargs) 

def query_local(**kargs):
    """
    Question 
    --------
    1. Given the stratified documents find the subsets (and their statistics) that contain CKI. 
    ...
    """
    def is_case(seq, code_prefix='584', code_set=[], threshold=1): # 584.x 
        return q.isCase(seq, code_set=code_set, code_prefix=code_prefix, logic='any', threshold=threshold)
    def get_global_cohort_dir(name='CKD'): 
        return seqparams.getCohortGlobalDir(name) # sys_config.read('DataExpRoot')/<cohort> 
    def stratum_stats(units): # <- units
        labels = units.keys() # documents = D; timestamps = T
    ...
    def policy_relabel():  
        ...
        return lmap
    def summary(adict): 
        for label, entries in adict.items(): 
            print('> CKD: %s' % label)
            for k, v in entries.items(): 
                print('  ... %s => %s' % (k, v)) 
        return
    from pattern import query as q
    from sampler import sampling 
    
    ### parameters 
    # A. Documents 
    simplifyCode = False
    cohort_name = kargs.get('cohort', lcsHandler.cohort)  # PTSD for experimenting on LCS-based labeling
    seq_ptype = kargs.get('seq_ptype', lcsHandler.ctype) # seqparams.normalize_ctype('diag') # opt: 'regular', 'diag', 'med'
    slice_policy = kargs.get('slice_policy', lcsHandler.slice_policy)

    # A.1 source 
    inputdir = kargs.get('inputdir', seqparams.getCohortGlobalDir(cohort='CKD')) # CKD0: smaller annotated cohort n=2833
    ifiles = kargs.get('ifiles', ['condition_drug_labeled_seq-CKD.csv', ])
    ...
    stratified = stratifyDocuments(cohort=cohort_name, seq_ptype=seq_ptype,
                    ...
                    # relabeling operation 
                    label_map=lmap,  # use default labeling
    ...
    nD = nT = 0
    labelToLCS = dict()
    lcsType, lcsPolicy = 'local', kargs.get('lcs_policy', 'df')
    patternType = kargs.get('pattern_type', 'enriched')
    
    comorbidPrefix = '584'
    comorbidities = [] 
    stageCases = {label: {} for label in stratified.keys()}
    for label, entry in stratified.items():
        ...
        n_comorbid_cases = 0
        for di in Di: 
            if is_case(di, code_prefix=comorbidPrefix, code_set=comorbidities, threshold=1): 
                n_comorbid_cases += 1
        
        stageCases[label]['n_total'] = len(Di)
        stageCases[label]['n_comorbid_cases'] = n_comorbid_cases
        stageCases[label]['ratio'] = n_comorbid_cases/(len(Di)+0.0)

    summary(stageCases)

    return

def evalCanonicalLabels(**kargs): 
    ### parameters 
    # A. Documents 
    simplifyCode = False
    cohort_name = kargs.get('cohort', lcsHandler.cohort)  # PTSD for experimenting on LCS-based labeling
    ...
    # A.1 source 
    inputdir = kargs.get('inputdir', get_global_cohort_dir(name='CKD0')) # CKD0: smaller annotated cohort n=2833
    ifiles = kargs.get('ifiles', ['condition_drug_labeled_seq-CKD.csv', ])
    ...
    stratified = stratifyDocuments(cohort=cohort_name, seq_ptype=seq_ptype,
                    ...
                    cutpoint=kargs.get('cutpoint', None),
                    inclusive=True)
    # labels = [label for label, entry in stratified.items()]
...
def t_lcs_local(**kargs):
    return runExperimentLocallyDerivedLCS(**kargs) 
def runExperimentLocallyDerivedLCS(**kargs): 
    """
    Wrapper of t_lcs() that operates on multiple experimental settings. 
    """
    import itertools 
    
    # ct0 = lcsHandler.ctype # to restore to the original 

    ct = ctypes = kargs.get('feature_types', ['regular', 'diag', 'med', ]) 
    pt = lcsHandler.pattern_types # ['enriched', 'rare']
    settings = [ct, pt, ]

    for setting in itertools.product(*settings):
        ctype, pattern_type = setting  # [hardcode]
        # lcsHandler.set_sequence_type(seq_type)

        # [note] need to set lcsHandler.ctype because naming of LCS-pattern file depends on it (see lcsHandler.lcs_load() )
        lcsHandler.ctype = kargs['seq_ptype'] = ctype  # document sequence type 
        kargs['pattern_type'] = pattern_type

        print('runExperimentLocallyDerivedLCS> setting | ctype=%s, pattern_type=%s' % (ctype, pattern_type))
        t_lcs(**kargs)  

    # restore to the original setting 
    # lcsHandler.ctype = ct0
    return
def t_lcs(**kargs): 
    """
    ...
    Memo
    ----
    1. makeLCSTSet common parameters: 
                topn_lcs=5, min_length=6, max_length=25, min_ndocs=2, 
                max_n_pairs=250000, pairing_policy='random', 
                load_lcs=False, 
                slice_policy='noop', slice_predicate=None, 
                consolidate_lcs=True

    2. return value from analyzeMCS(): 
    ...
    def analyze_the_other_strata(current_label, df_lcs, pattern_type='enriched', save_=True, load_analyzer_result=True, header=[]):  
        # df_lcs: dataframe returned from deriveLCS() 
        #         columns: ['length', 'lcs', 'n_uniq', 'count', 'df', ]
        # lcs_set = df_lcs['lcs']
        ctype = kargs.get('seq_ptype', lcsHandler.ctype)
        if not header: header = ['lcs', ] + [label for label in stratified.keys()] # lcs, counts in label1, counts in label2, ... counts in label N
        
        adict = {h:[] for h in header}   
        
        # LCS variable set
        lcs_set = adict['lcs'] = df_lcs['lcs']  # reference lcs set
        adict[current_label] = df_lcs['count']
        print('(analyze_the_other_strata) size(lcs_set): %d' % len(lcs_set))

        nD = nT = 0
        # counts = {}
        for label, entry in stratified.items(): 
            if label == current_label: 
                continue   # LCS docfreq for the current stratum has been stored in df_lcs['count']

            # if not counts.has_key(label): counts[label] = []
            Di = entry['sequence']
            Ti = entry['timestamp']
            Li = entry['label']
            nD += len(Di)
            nT += len(Ti)
            print('  + searching LCSs: current label: %s, other label: %s | N=%d ... ' % (current_label, label, len(Di)))

            # query frequency: number of docs in the other strata in which lcs occurs
            # [note] candiate attributes: ['length', 'lcs', 'n_uniq', 'count', 'df', ]
            ret = analyzeMCS(Di, Ti, lcs_set=lcs_set, make_color_time=False, check_df_min=False, 
                    save_result=True, load_result=load_analyzer_result, 
                    outputfile='analyze_strata-%s-CL%s-L%s.pkl' % (ctype, current_label, lcsHandler.process_label(label)))  # index:lcsmap, frequency:lcsmapInvFreq
            
            # polling each LCS var for its docfreq in this stratum
            for lcs in lcs_set: 
                ids = ret['index'].get(lcs, [])# lcsmap: lcs -> document IDs 
                ndocs = len(ids) # number of documents that contain this LCS
                adict[label].append(ndocs)
        df_rel = DataFrame(adict, columns=header)
        if save_: save_relative_stats(df_rel, label=current_label, meta=pattern_type)
        return df_rel

    def load_relative_stats(label, meta='enriched'): # [note] for now, only used in makeLCSTSet()
        # e.g. lcs_local-df-LCKDStage2-Urel-enriched.csv  ... file name for relative stats
        #      lcs_local-df-diag-iso-LCKDStage3.csv ... the source from which this relative stats are derived 
        cohort = kargs.get('cohort', lcsHandler.cohort)  
        seq_ptype = kargs.get('seq_ptype', lcsHandler.ctype)
        consolidate_lcs = kargs.get('consolidate_lcs', 'iso')

        ltype = 'local'  # from global to local
        lcs_select_policy = kargs.get('lcs_policy', lcsHandler.lcs_policy)
        suffix = '%s-%s-L%s-Urel-%s' % (seq_ptype, consolidate_lcs, label, meta)
        return Pathway.load(cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix, dir_type='pathway')
    def save_relative_stats(df, label, meta='enriched'):  
        # statistics (e.g. frequency of LCSs) relative to the input label (i.e. using label)
        cohort = kargs.get('cohort', lcsHandler.cohort)  
        seq_ptype = kargs.get('seq_ptype', lcsHandler.ctype)
        consolidate_lcs = kargs.get('consolidate_lcs', 'iso')
        ...
        ltype = 'local' # from global to local
        lcs_select_policy = kargs.get('lcs_policy', lcsHandler.lcs_policy)
        suffix = '%s-%s-L%s-Urel-%s' % (seq_ptype, consolidate_lcs, label, meta)  # rel: relatively     

        # fpath = Pathway.getFullPath(cohort=cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix) # [params] dir_type='pathway' 
        print('t_lcs> saving relative LCS stats ...')  # e.g. lcs_local-df-regular-iso-LControl-Urel-enriched.csv
        Pathway.save(df, cohort=cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix, dir_type='pathway')
        return   
    def select_lcs_set(df, max_n=None, pattern_type='enriched', criteria=['count', ]):
        if max_n is None: max_n = topNLCS  

        ascending = True if pattern_type.startswith('ra') else False
        df = df.sort_values(criteria, ascending=ascending).head(max_n)  # [note] 'count'/frequency determines if an lcs being enriched or rare
        if df.shape[0] < max_n: print('(select_lcs_set) n: %d but desired: %d (max_n)' % (df.shape[0], max_n))
    ...
    def remove_diag_prefix(D): 
        ...
        for i, doc in enumerate(D):  # D: a list of list of tokens 
            ...
            D[i] = map(remove_prefix, doc)
        return
    def remove_prefix(x, regex='^I(10|9):'):
        return re.sub(regex, '', x)

    from sampler import sampling 
    # from pattern import medcode as med
    ...
    # B. LCS 
    topNLCS = 1000
    nLCSLocal = 10000  # try to identify at least this many patterns before skipping deriveLCS()
    ...
    stratified = stratifyDocuments(cohort=cohort_name, seq_ptype=seq_ptype,
                    ...
                    # remove prefixes (new odhsi diag code format contains prefixes e.g. I9, I10)
                    remove_cond_prefix=True, 
                    remove_med_prefix=False, 
    ...
    # predicates 
    tLoad = kargs.get('load_precomputed_lcs', True)
    tLoadAnalyzerResult = True
    tComputeLCS = False

    nD = nT = nLCS0 = 0
    labelToLCS = dict()
    lcsType = 'local'
    lcsPolicy = kargs.get('lcs_policy', 'df')
    patternType = kargs.get('pattern_type', 'enriched')
    header_analyzer = ['lcs', ] + [label for label in sorted(stratified.keys())] # lcs, counts in label1, counts in label2, ... counts in label N
    ...
    for label, entry in stratified.items():
        ...
        # [note] except for label, other parameters are configured by lcsHandler
        #        e.g. lcs_local-df-regular-iso-LControl-Uenriched.csv
        df = DataFrame() # dummy
        if tLoad: 
            df = lcsHandler.load_lcs(label=process_label(label), 
                    lcs_type=lcsType, lcs_policy=lcsPolicy, meta=patternType, seq_ptype=seq_ptype) # lcs_type=lcsType, lcs_policy=lcsPolicy
            nLCS0= df.shape[0] if df is not None else 0   
           
        
        nLCSDelta = nLCSLocal - nLCS0         
        if nLCSDelta > 0:
            nLCSOffset = 20 
            tLoadAnalyzerResult = False
            print('(t_lcs) label: %s | precomputed df has %d < %d patterns > adding %d new ones ...' % (label, nLCS0, nLCSLocal, nLCSDelta))
            # ['length', 'lcs', 'n_uniq', 'count', 'df', ] 
            df = deriveLCS(D=Di, 
                    # inputdir=inputdir, ifiles=ifiles,
                    
                    topn_lcs=kargs.get('topn_lcs', nLCSDelta+nLCSOffset), # "topN" for enriched and rare
                    ...
                    lcs_type=lcsType, 
                    lcs_policy=lcsPolicy, # lcs filtering policy

                    ### update rule 
                    # if True, do load_lcs() first and add new LCS patterns to it (always True in deriveLCS2())
                    incremental_update=kargs.get('incremental_update', True), 
                    return_type=kargs.get('return_type', patternType), # values: {'enriched', 'rare', }

                    # parameters defining sequence properties (no longer necessary; specified through lcsHandler)
                    seq_ptype=seq_ptype, 
                    slice_policy=slice_policy,  # same as the param in stratifyDocuments; only for file ID purpose 
                    cohort=cohort_name) # used for file ID and op such as filter_by_anchor()
        
            # save LCS stats to: Pathway.save(df, cohort=cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix, dir_type='pathway')
            # e.g.   <prefix>/tpheno/seqmaker/data/CKD/pathway/lcs_local-df-diag-iso-LControl-Urare.csv
            ...
            df = select_lcs_set(df, max_n=topNLCS, pattern_type=patternType, criteria=['count', ])
        else: 
            print('(t_lcs) label: %s | precomputed dataframe contain sufficient patterns > n=%d' % (label, df.shape[0]))

        # most enriched LCS in each stratium? 
        # labelToLCS[label] = lcs_set
        # assert len(lcs_set) == topNLCS, "size of lcs: %d but desired (topNLCS): %d" % (len(lcs_set), topNLCS)
        
        # how often do these LCSs occur in other clusters/strata? 
        analyze_the_other_strata(current_label=label, df_lcs=df, pattern_type=patternType, save_=True, 
            load_analyzer_result=tLoadAnalyzerResult, header=header_analyzer)

        ### time series, colors  ... expensive! 
...
def queryDocuments(**kargs): 
    """
    Query document properties 

    Use
    ---
    1. example sequences given a label


    Example
    -------
    queryDocuments(cohort='CKD', use_dev=True, policy_segment='prior', predicate=ckd.isCaseCCS)
    ...
    def analyze_the_other_strata(current_label, df_lcs, pattern_type='enriched', save_=True, load_analyzer_result=True):  
        # df_lcs: dataframe returned from deriveLCS() 
        #         columns: ['length', 'lcs', 'n_uniq', 'count', 'df', ]
        # lcs_set = df_lcs['lcs']
        ctype = kargs.get('seq_ptype', lcsHandler.ctype)
        header = ['lcs', ] + [label for label in stratified.keys()] # lcs, counts in label1, counts in label2, ... counts in label N
    ...
    def remove_prefix(x, regex='^I(10|9):'):
        return re.sub(regex, '', x)
    def sample_sequences(di, ti, label='n/a', n=20, maxlen=100):
        N = len(di)
        sidx = random.sample(range(N), min(n, N))
        print('\n(sample_sequences) Showing sample sequences for label: %s\n' % label)
        for i, r in enumerate(sidx):
            length = len(di[r])
            if length > maxlen:
                # splice the sequence  
                offset = maxlen/2
                # firsthalf, secondhalf = di[r][:lower], di[r][-upper:]
                s1 = ' '.join([str(e) for e in di[r][:offset]])
                s2 = ' '.join([str(e) for e in di[r][-offset:]])
                print('%s [%d] %s\n... %s\n' % (label, i, 'excerpted', s1 + '...' + s2))  # abridged versino of the sequence
            else: 
                s0 = ' '.join([str(e) for e in di[r]])
                print('%s [%d]\n... %s\n' % (label, i, s0))
        return 
    ...
    predicate_default = None if policy_segment.startswith('reg') else ckd.isCaseCCS   # same as ckd.isCase plus minor error correction

    tUseDevData = kargs.get('use_dev', True)
    # inputfile = kargs.get('inputfile', 'condition_drug_labeled_seq-CKD.csv') 

    default_prefix = seqparams.getCohortGlobalDir(cohort=cohort_name) # small cohort (n ~ 2.8K)
    inputdir = kargs.get('inputdir', default_prefix) # seqparams.getCohortGlobalDir(cohort='CKD0') # small cohort (n ~ 2.8K)
    ifiles = kargs.get('ifiles', ['condition_drug_labeled_seq-%s.csv' % cohort_name, ])
    # kargs['segment_by_visit'] = kargs.get('segment_by_visit', False)
    ...
    # A.1 source 
    default_prefix = seqparams.getCohortGlobalDir(cohort=cohort_name) # small cohort (n ~ 2.8K)
    if tUseDevData: inputdir = select_data_source(cohort=cohort_name, source_type='dev') # CKD0: smaller annotated cohort n=2833
    ...
    stratified = stratifyDocuments(cohort=cohort_name, seq_ptype=seq_ptype,
                    ...
                    predicate=kargs.get('predicate', predicate_default),
    ...
    tShowSamples = kargs.get('show_samples', True)
    ...
    for label, entry in stratified.items():
        ...
        nD += len(Di)
        nT += len(Ti)
        print('queryDocuments> label=%s | N=%d' % (label, len(Di)))

        minLCSDocFreq = 2 # min(len(Di)/10, 2)

        if tShowSamples: sample_sequences(di=Di, ti=Ti, label=label, n=20)
        
    return nD

def findRepresentativeLCSPerStage(**kargs):
    def process_label(l): 
        # remove spaces e.g. CKD Stage 4
        return ''.join(str(e) for e in l.split())
    def policy_relabel():  
        ...
        return lmap
    def load_relative_stats(label, ctype='regular', meta='enriched'): # [note] for now, only used in makeLCSTSet()
        # params
        #    ctype: {'regular', 'diag', 'med', }
        #  
        print('(load_relative_stats) loading label=%s, ctype=%s, meta=%s ...' % (label, ctype, meta))
        # e.g. lcs_local-df-LCKDStage2-Urel-enriched.csv  ... file name for relative stats
        #      lcs_local-df-diag-iso-LCKDStage3.csv ... the source from which this relative stats are derived 
        cohort = kargs.get('cohort', lcsHandler.cohort)  
        seq_ptype = kargs.get('seq_ptype', ctype) # don't use the system's default
        consolidate_lcs = kargs.get('consolidate_lcs', 'iso')
        ...
        suffix = '%s-%s-L%s-Urel-%s' % (seq_ptype, consolidate_lcs, label, meta)
        return Pathway.load(cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix, dir_type='pathway')
    def save_relative_stats(df, label, ctype='regular', meta='enriched'):  
        # params
        #    ctype: {'regular', 'diag', 'med', }
        #    
        ...
        # fpath = Pathway.getFullPath(cohort=cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix) # [params] dir_type='pathway' 
        print('findRepresentativeLCSPerStage> Saving relative LCS stats ...')  # e.g. lcs_local-df-regular-iso-LControl-Urel-enriched.csv
        Pathway.save(df, cohort=cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix, dir_type='pathway')
        return
    def is_null(df):
        return df is None or df.empty 
    def make_hashtb(entries): 
        adict = {}
        for entry in entries: 
            adict[entry] = []
        return
    ...
    # A.1 source 
    inputdir = kargs.get('inputdir', seqparams.getCohortGlobalDir(cohort_name)) # get_global_cohort_dir(name='CKD'); CKD0: smaller annotated cohort n=2833
    ifiles = kargs.get('ifiles', ['condition_drug_labeled_seq-CKD.csv', ])
    ...
    labels = ["Control", 
              ...
              "CKD Stage 3", "CKD Stage 4", "CKD Stage 5"] 
    col_pattern = 'lcs'
    header = [col_pattern, ] + labels
    pattern_types = ['enriched', 'rare', ]
    ctypes = ['diag', 'med', 'regular', ] # 'med','regular'
    ...
    for pt in pattern_types:
        for ctype in ctypes: 
            ...
            hasData = False
            for label in labels: 
                lr = lcsHandler.process_label(label) 
                df = load_relative_stats(label=lr, ctype=ctype, meta=pt)

                # assert not is_null(df), 'null dataframe'
                if df is not None and not df.empty: 
                
                    # find characteristic LCSs for each stage

                    # header = ['lcs', ] + [label for label in stratified.keys()] # lcs, counts in label1, counts in label2, ... counts in label N
                    df_total = df.drop([col_pattern, ], axis=1).sum(axis=1)  # axis=1, (sum) across columns
                    df['ratio'] = df[label]/(df_total+0.0)
                    save_relative_stats(df, label=lr, ctype=ctype, meta=pt)
                    hasData = True
                else: 
                    hasData = False

                if not hasData: 
                    print('(query) No data available yet for ctype: %s, pattern type: %s' % (ctype, pt))
                    break


    # run a set of queries
    # 1. top 20 most representative LCSs for each stage? 
    adict = make_hashtb(labels)
    sort_attributes = ['ratio', ]
    topN = None
    for label in labels: 
        for pt in pattern_types:
            for ctype in ctypes: 
                lr = lcsHandler.process_label(label) 
                df = load_relative_stats(label=lr, ctype=ctype, meta=pt)
                assert not is_null(df), 'null dataframe'; assert 'ratio' in df.columns, 'invalid dataframe > df.columns=%s' % df.columns.values
                # find characteristic LCSs for each stage
                # header = ['lcs', ] + [label for label in stratified.keys()] # lcs, counts in label1, counts in label2, ... counts in label N
...
def t_lcs_global(**kargs):
    return runExperimentGloballyDerivedLCS(**kargs)
def runExperimentGloballyDerivedLCS(**kargs):  # global counterpart of runExperimentLocallyDerivedLCS() 
    """
    Wrapper of t_lcs2() that operates on multiple experimental settings. 
    """
    import itertools 
    ...
    for setting in itertools.product(*settings):
        ctype, pattern_type = setting  # [hardcode]
        # lcsHandler.set_sequence_type(seq_type)
        kargs['seq_ptype'] = ctype  # document sequence type 
        kargs['pattern_type'] = pattern_type
        print('runExperimentGloballyDerivedLCS> setting | ctype=%s, pattern_type=%s' % (ctype, pattern_type))
        t_lcs2(**kargs)  
...
def t_lcs2(**kargs):
    ...
    def get_tset_dir(): # derived MDS 
        ...
        return tsetPath 
    def analyze_strata(stratified, current_label, lcs_candidates, topn=100, raren=100, max_n_samples=None, 
                        save_=True, load_=False, return_type='rare', save_intermediate=True):  
        # df_lcs: dataframe returned from deriveLCS() 
        #         columns: ['length', 'lcs', 'n_uniq', 'count', 'df', ]
        # lcs_candidates = df_lcs['lcs']
        
        ctype = kargs.get('seq_ptype', lcsHandler.ctype)
        header = ['lcs', ] + [label for label in stratified.keys()] # lcs, counts in label1, counts in label2, ... counts in label N
        # adict = {h:[] for h in header}  
        
        # default lcs candidates
        n_samples = len(lcs_candidates)  # min(topn, df_lcs.shape[0])
        
        # adict[current_label] = df_lcs['count']
        if max_n_samples is None: 
            pass
        else: 
            lcs_candidates = random.sample(lcs_candidates, max_n_samples)
            n_samples = len(lcs_candidates)
        print('  + examining %d candidate LCSs ...' % n_samples)
        
        # first, get reference LCS set (e.g. top N most enriched LCSs or rarest LCSs)
        entry = stratified[current_label]  
        Dr = entry['sequence']  
        Tr = entry['timestamp'] 
        Lr = entry['label']  

        # [note] expensive! save intermmediate result if possible
        outputfile = 'analyze_strata-%s.pkl' % ctype
        tAnalyzedMCS = False
        ret =  analyzeMCS(Dr, Tr, lcs_set=lcs_candidates, make_color_time=False, check_df_min=False, 
                    save_result=True, load_result=True, outputfile=outputfile)  # index:lcsmap, frequency:lcsmapInvFreq
        
        candidates = []
        for lcs in lcs_candidates: 
            ids = ret['index'][lcs]  # lcsmap: lcs -> document IDs 
            candidates.append((lcs, len(ids)))

        rareLCSs = sorted(candidates, key=lambda x: x[1], reverse=False)[:raren] # least frequent
        freqLCSs = sorted(candidates, key=lambda x: x[1], reverse=True)[:topn]  # most frequent
        
        settings = [('enriched', freqLCSs), ('rare', rareLCSs), ]
        dfr = df_rel = None
        for setting in settings: 

            meta = setting[0]
            rankedLCSs = setting[1]

            hasPrecomputed = False
            if load_: # load precomputed data set if possible (since analyzeMCS is time-consuming)
                df_rel = load_relative_stats(label=current_label, meta=meta) 
                if df_rel is not None and not df_rel.empty: 
                    hasPrecomputed = True
                    print('  + found precomputed data | n_entries: %d' % df_rel.shape[0])
            
            if not hasPrecomputed: 
                adict = {h:[] for h in header} 
                lcs_set = adict['lcs'] = [lcs for lcs, freq in rankedLCSs]
                adict[current_label] = [freq for lcs, freq in rankedLCSs]  # number of occurrences 

                nD = nT = 0

                # among the enriched/rare LCS patterns (of current label), how often do they occur in other stages? 
                for label, entry in stratified.items(): 
                    if label == current_label: continue
                    ...
                    # query frequency: number of docs in the other strata in which lcs occurs
                    # [note] candiate attributes: ['length', 'lcs', 'n_uniq', 'count', 'df', ]
                    outputfile = outputfile = 'analyze_strata-%s-CL%s-L%s.pkl' % (ctype, current_label, label)
                    ret = analyzeMCS(Di, Ti, lcs_set=lcs_set, make_color_time=False, check_df_min=False, 
                            save_result=True, load_result=True, outputfile=outputfile)  # index:lcsmap, frequency:lcsmapInvFreq
                ...
                df_rel = DataFrame(adict, columns=header)
                if save_: save_relative_stats(df_rel, label=current_label, meta=meta)
            
            if meta.startswith(return_type): dfr = df_rel 
    ...
    def load_relative_stats(label, meta='enriched'): # [note] for now, only used in makeLCSTSet()
        ...
        ltype = 'global'  # from global to local
        lcs_select_policy = kargs.get('lcs_policy', 'df')
        suffix = '%s-%s-L%s-Urel-%s' % (seq_ptype, consolidate_lcs, label, meta) 
    ...
    def save_relative_stats(df, label, meta='enriched'):  
        ...
        print('t_lcs2> saving relative LCS statistcs ...')
        Pathway.save(df, cohort=cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix, dir_type='pathway')
        return 
    def select_lcs_set(df, max_n=None, pattern_type='enriched', criteria=['count', ]):
        # use: df = select_lcs_set(df, max_n=topNLCS, pattern_type=patternType, criteria=['count', ])
    ...
    simplifyCode = False
    cohort_name = kargs.get('cohort', lcsHandler.cohort)  # PTSD for experimenting on LCS-based labeling
    ctype = seq_ptype = kargs.get('seq_ptype', lcsHandler.ctype) # seqparams.normalize_ctype('diag') # opt: 'regular', 'diag', 'med'
    slice_policy = kargs.get('slice_policy', lcsHandler.slice_policy)
    ...
    # sq.sysConfig(cohort=cohort_name, seq_ptype=ctype, 
    #     lcs_type='global', lcs_policy='df', 
    #     consolidate_lcs=True, 
    #     slice_policy=slice_policy, 

    #     simplify_code=simplifyCode, 
    #     meta=userFileID)

    minDocLength = 10  # only retain documents with at least 10 codes
    maxNDocs = 10000   # derive LCSs only from this many documents (speed), but 'count' will not be correct
    ...
    for i in range(nTrials): 
        precondition(i)
        df, summary = docToLCS(lcsHandler.cohort, 
    ...
    # [note] 1. since we gathered a lot of LCSs, presumably, no need to distinguish pattern types (i.e. enriched vs rare)
    #           => set patternType to None by setting meta to None # no explicit pattern type
    #        2. 
    dfG = lcsHandler.load_lcs(lcs_type=lcsType, lcs_policy=lcsPolicy, meta=None, seq_ptype=ctype) # (absolute) global
    assert dfG is not None and not dfG.empty
    max_n_samples = min(20000, dfG.shape[0]) 
    lcsCandidates = dfG.sample(n=max_n_samples)['lcs'] # dfG['lcs'].values
    print('  + Sampled %d global LCS canidates out of totally %d patterns ...' % (len(lcsCandidates), dfG.shape[0])) # [note] got 43406 as of 07.18
    ...
    for label, entry in stratified.items():
        ...
        ### skip local operation: df = deriveLCS(...)  # ['length', 'lcs', 'n_uniq', 'count', 'df', ] 
        # df = deriveNGram(Di, 
        #                 topn=100,   # retain only topn n-grams for each length (n)
        #                 min_length=1, max_length=8, partial_order=False,
        #                 ng_type='local',
        ...
        # how often do these LCSs occur in other clusters/strata? 
        # cf: t_lcs() analyze_the_other_strat()
        #     in this case, we find, from among the global LCS, the topn (say 100) in the current label first 
        #     use these as references, and subsequently, find the frequencies at which they occur in other strata
        analyze_strata(stratified, current_label=label, lcs_candidates=lcsCandidates, topn=topNLocal, raren=rareNLocal, 
            save_=True, load_=kargs.get('load_lcs_stats', True)) 
...
def t_make_lcs_fset(**kargs):
    ...
    def summary(): 
        ...
        return
    def config(**kargs): 
        sq.sysConfig(cohort=kargs.get('cohort', 'CKD'), seq_ptype=kargs.get('seq_ptype', 'regular'), 
            lcs_type=kargs.get('lcs_type', 'global'), lcs_policy=kargs.get('lcs_policy', 'df'), 
            consolidate_lcs=True, 
            slice_policy=kargs.get('slice_policy', 'noop'), 
            simplify_code=kargs.get('simplify_code', False), 
    ...
    # [note] these system and cohort paramters should have been configured prior to the call
    cohort = kargs.get('cohort', lcsHandler.cohort)
    ...
    # lcsType, lcsPolicy = kargs.get('lcs_type', 'global'), kargs.get('lcs_policy', 'df')
    ...
    if tCompleteData: 
        if tMakeTSet:
            kargs['topn'] = topn = kargs.get('topn', 10000)  # select a total of this many features
            kargs['pattern_type'] = kargs.get('pattern_type', 'rare')

            # [test]
            print('(t_make_lcs_fset) Verifying input params ...')
            # uncommon params: ifiles, 
            for param in ('inputdir', 'inputfile', 'lcs_var_policy', 'topn', 'pattern_type', 'lcs_policy', 'feature_types' ): 
                if kargs.has_key(param): 
                    print('... %s: %s' % (param, kargs[param]))
            print('(t_make_lcs_fset) Computing feature set with a complete document set ...')
            # X, y = makeLCSFeatureTSet2a(cohort, **kargs)  # if non-incremental, use makeLCSFeatureTSet2(cohort, **kargs) 
            
            # load globally derived feature set, analyze their occurrences, followed by training set creation
            X, y = makeLCSFeatureTSet3(cohort, **kargs)  # globally derived feature set
            summary()
        
        # [note] this is subsumed by analyze_the_other_strata()/local and analyze_strata()/global
        print('(t_make_lcs_fset) analyze lcs distributions in different strata ... ')
        ...
        print('(t_make_lcs_fset) sampling a subset of documents (n=%d)' % max_ndocs)
...
def t_characterize_strata(**kargs): 
...
def t_analyze_mcs(**kargs):  
    ...
    D, L, T = process_docs()  # set inputdir to None to use default
    nD = len(D)
...
def t_model(**kargs):
    ...
    if tMakeLCSTSet: 
        ...
        ts = makeLCSTSet(cohort=cohort_name, d2v_method=d2v_method, seq_ptype=seq_ptype, 
                topn_lcs=5, min_length=5, max_length=25, min_ndocs=2, 
                max_n_pairs=250000, pairing_policy='random', 
                load_lcs=False, 
...
def featureSelect(topn, **kargs): 
    ...
    def load_tset(n_per_class=None): 
        X, y = kargs.get('X', None), kargs.get('y', None)
        fset = kargs.get('header', [])
        # e.g. tset-IDregular-pv-dm2-Rlcs-sorted-GCKD.csv 
        if X is None or y is None: 
            X, y, fset = loadLCSFeatureTSet(scale_=kargs.get('scale_', True), 
                            label_map=seqparams.System.label_map, 
                            drop_ctrl=kargs.get('drop_ctrl', True), 
                            n_per_class=n_per_class) # subsamping
        assert X is not None
    ...
    ### (X, y, fset)
    X, y, lcsFSetTotal = load_tset(n_per_class=kargs.get('n_per_class', None))
    ...
    # clf_default = RandomForestClassifier(n_estimators=5000, random_state=53, n_jobs=15)  # avg importance: 2.304e-05
...
def t_classify(**kargs):
    ...
    clf, param_grid = choose_classifier(name=kargs.pop('clf_name', 'gradientboost')) # rf: random forest
    ...
    maxSampleRatios = kargs.get('ratios', []) # [0.9, 0.1, ]

    patternType = kargs.get('pattern_type', 'rare') # using enriched LCS or rare LCS as variables? 
    if ts_dtype.startswith('d'): 
        ...
        fset = kargs.get('header', [])
        if X is None or y is None: 
            X, y, fset = loadTSet(scale_=kargs.get('scale_', False), 
        ...
        # [usage] 1. following featureSelect* routine only works for non-sparse data
        # if kargs.get('apply_fs', False): 
        #     # [params] estimator
        ...
        nTrials = kargs.get('n_trials', 10)

        result_set = []
        for clf in clf_list: 
            res = sclf.modelEvaluateBatch(X, y, 
                    classifier=clf, 
            ...
            # res = sclf.multiClassEvaluateSparse(X=X, y=y, 
            #         classifier=clf, 
...
def t_lcs_feature_select(**kargs):
    ...
    if True: 
        X, y = kargs.get('X', None), kargs.get('y', None)
        if X is None or y is None: 
            X, y = loadTSet(scale_=kargs.get('scale_', True), 
                label_map=seqparams.System.label_map, 
                drop_ctrl=kargs.get('drop_ctrl', True), 
                n_per_class=maxNPerClass) # subsamping
        else: 
    ...
    # select features based on importance weights dervied from lasso
    X, y, fset = featureSelect(topn=topNFSet, X=X, y=y, policy_fs='sorted', 
                    n_sample_per_class=maxNSamplePerClass, load_selected=kargs.get('load_selected_lcs', False)) 
...
def runWorkFlow(**kargs):
    ...
    ctype = seqparams.normalize_ctype('regular')  # {'regular', diag', 'med'} 
    sq.sysConfig(cohort=cohort, seq_ptype=ctype, 
        lcs_type='global', lcs_policy='df', 
        consolidate_lcs=True, 
        slice_policy=kargs.get('slice_policy', 'noop'), 
    ...
    ### configuration 
    # a) pairwise LCS patterns from a subset of the given cohort
    tComputeLocalLCS = kargs.get('derive_local_lcs', False)
    tComputeGlobalLCS = kargs.get('derive_global_lcs', False) 

    # b) predictive analytics using LCS 'feature set'
    var_scope = kargs.get('lcs_var_policy', 'local') # {'local', 'global', } 
    pattern_type = kargs.get('pattern_type', 'rare') # {'enriched', 'rare', }
    n_features = kargs.get('n_features', 10000)

    # c) use the identified LCS patterns as features to stratify the given cohort 
    tClassify = kargs.get('enable_classifier', False)

    # d) answer particular queries, research questions
    tQuery = kargs.get('answer_queries', False)
    ...
    # name: {'CKD0', 'CKD', }
    #       large cohort (n=657591 ~ 650K): CKD, dev cohort (n=2833): CKD0  
    inputdir = get_global_cohort_dir(name='CKD')  
    feature_types = kargs.get('feature_types', ['regular', ] )  # ['med', 'diag', ], ['regular', ]
    
    # extract LCS patterns from within each stratification (of a given cohort)
    #     e.g. find enriched LCSs from the stage-V subgroup in a CKD cohort
    if tComputeLocalLCS: 
        # [note] derive all feature types: regular, med, diag but later on, use regular LCS patterns to make predictions
        runExperimentLocallyDerivedLCS(inputdir=inputdir, feature_types=['regular', 'med', 'diag'])  # i.e. t_lcs_local(): derive local LCSs from within each stratum

    ### global -> stratified LCS patterns
    if tComputeGlobalLCS: 
        runExperimentGloballyDerivedLCS(inputdir=inputdir, load_lcs_stats=True, feature_types=feature_types)  
    ...
    ### Classification: create LCS feature set given LCSs derived from t_lcs2() 
    if tClassify: 
        ...
        # [note] lcs_type: 'global' by default (feature set derived via runExperimentGloballyDerivedLCS)
        # [params] {'inputdir', 'inputfile', 'lcs_var_policy', 'topn', 'pattern_type', 'lcs_policy'}
        t_make_lcs_fset(inputdir=inputdir, lcs_var_policy=var_scope, 
            pattern_type=pattern_type, topn=n_features, feature_types=feature_types) # i.e. t_lcs2a() | default: topn=10K, pattern_type='rare'

        tLoadSelectedLCS, tApplyFS = True, False
        tDropCtrl = False
        nTrials = 1
    ...
    ### Misc Research Questions 
    # query_local()

    ## most representative LCSs can be defined as follows: 
    #  i) Given a subgroup (e.g. CKD stage V), LCSs that frequently occur this subgroup but less so in the others
    #     the higher the contrast, the higher its representativeness 
    #  ii)  
    if tQuery:  

        # analyzeLCSDistribution(cohort, **kargs)

        # use this to find representative LCSs and relative statistics comparing one stratum to the other strata
        # findRepresentativeLCSPerStage(**kargs) 

        # use this to get example sequences, 
        # [params] predicate=ckd.isCaseCCS   
        #    predicate may cause problems
        N = queryDocuments(cohort='CKD', use_dev=True, seq_ptype='regular', policy_segment='regular', show_samples=True)
        Nprior = queryDocuments(cohort='CKD', use_dev=True, seq_ptype='regular', 
                    policy_segment='prior', predicate=ckd.isCaseCCS, show_examples=False)
        ratio = Nprior/(N+0.0)
        print('runWorkFlow> ratio of docs containing disease distinguishing diag codes: %f | N=%d, Nprior=%d' % (ratio, N, Nprior)) 
...
if __name__ == "__main__": 
    runWorkFlow(derive_local_lcs=True, enable_classifier=True, answer_queries=True)
    # test_batch()
