...
class lcsHandler(seqConfig.lcsHandler): 
    ...
    def load_lcs(**kargs): # [note] for now, only used in makeLCSTSet()
        ...
        suffix = adict['meta'] 
        print('  + pathway params > ltype: %s, lcs_policy: %s, suffix/meta: %s' % (ltype, lcs_select_policy, suffix))
    ...
    @staticmethod
    def save_tset(X, y, policy_fs='sorted', meta=None):
        from tset import TSet
...
def analyzeMCS(D, T, lcs_set, **kargs):
    ...
    tSave = kargs.get('save_result', False)  # analyze_strata() may need this result
    if tSave: 
        fname = kargs.get('outputfile', 'analyzeMCS.pkl')
...
def deriveLCS(D, **kargs):
    ...
    def make_pairs(D_subset, cid=0, policy='random', n_pairs=100000): # [params] maxNPairs, policy={'longest_first', 'random'}
        ...
        n = len(D_subset)
        # pairwise lcs   
        n_total_pairs_est = n_total_pairs = (n*(n-1))/2.  # approx.
    ...
    for cid, documents in DCluster.items(): # unless specified otherwise, assume that all documents (D) are one big cluster
        ...
        n_total_pairs = len(pairs); assert n_total_pairs >  maxNPairs/2 # sum(1 for x in pairs) # if pairs is a generator, this exahusts it! 
...
def makeLCSFeatureTSet2(cohort, **kargs): 
    ...
    * Persistent 
    ...
    def save_tset(X, y, policy_fs='sorted', meta=None):
        ...
        # PS2: load method
        # X, y = TSet.loadSparseLCSFeatureTSet(cohort, **kargs)
        return lcsHandler.save_tset(X, y, policy_fs=policy_fs, meta=meta)
    def load_tset(policy_fs='sorted', meta=None):
    ...
    if kargs.get('save_', True): 
        save_tset(Xs, L, meta=kargs.get('pattern_type', 'enriched'))
...
def loadLCSFeatureTSet(**kargs):
    ...
    """
    def load_tset(policy_fs='sorted', meta=None):
        return lcsHandler.load_tset(policy_fs=policy_fs, meta=meta)  # meta: use pattern type by default
        # return (X, y)
    def modify_tset(X, y): 
    ...
    ### load 
    X, y = load_tset(meta=kargs.get('pattern_type', 'rare'))
    profile(X, y)  # initial 
...
def makeLCSFeatureTSet3(cohort, **kargs): 
    ...
    def process_docs(): 
        ...
        # Dl = labelize(D, label_type='doc', class_labels=L)  # use class labels for document label prefixing
        is_labeled = len(np.unique(L)) > 1
        print('  + nD: %d | cohort=%s, ctype=%s, labeled? %s, simplified? %s' % (len(D), lcsHandler.cohort, ctype, is_labeled, tSimplified))
        return (D, L, T)
    ...
    def eval_feature_value(D, L, fset, lcs_result, inputdir=None, content_sep=','): 
        ...
        for i, doc in enumerate(D):
            ...
            values = []
            # for lcs, tf in gTermFreq[i]: 
            for i, lcs in enumerate(fset): # fill the feature value according to input feature order
        ...
        print('makeLCSFeatureTSet2a> Found %d coordinates, %d active values | nD: %d >? nDEff: %d ' % \
    ...
    policyVars = kargs.get('lcs_var_policy', 'global')
    ...
    D, T, L = process_docs()  # need to know the labels in local feature mode
    labels = np.unique(L)
    if policyVars.startswith('gl'):  # use global feature set 
        lcsType = 'global'
        dfG = lcsHandler.load_lcs(lcs_type=lcsType, lcs_policy=lcsPolicy, meta=None)  # local global LCS candidates 
        # n_sample = dfG.shape[0]
    else: # local 
        lcsType = 'local'
        dfG = []
        for label in labels: 
            df = lcsHandler.load_lcs(label=lcsHandler.process_label(label), 
                    lcs_type=lcsType, lcs_policy=lcsPolicy, meta=patternType) # lcs_type=lcsType, lcs_policy=lcsPolicy
            dfG.append(df)
        dfG = pd.concat(dfG, ignore_index=True)
    nF0 = min(n_features, dfG.shape[0]) 
    print('... desired %d vars but effectively got %d' % (n_features, nF0))

    # use a smaller sample size to speed up
    # [note] the "count" statistics is not reliable due to subsampling
    lcsCandidates = dfG.sample(n=n_features_eff)['lcs'] # dfG['lcs'].values
    nF0 = len(lcsCandidates)
    print('... selected %d (from %d) LCS canidates ...' % (nF0, dfG.shape[0]))  # [note] got 43406 as of 07.18

    ### global analysis 
    ret = analyzeMCS(D, T, lcs_set=lcsCandidates, make_color_time=False, incremental_mode=False)  # index:lcsmap, frequency:lcsmapInvFreq

    # filter
    candidates = []
    minLCSDocFreq = kargs.get('min_ndocs', 2) 
    ...
    lcsCandidates = [f for f, cnt in sorted(candidates, key=lambda x: x[1], reverse=reverse)[:nF]] # least frequent
    print('  + got %d LCS features (pattern type=%s) < n_total=%d s.t. min df: %d' % (len(lcsCandidates), patternType, nF, minLCSDocFreq))

    # sparse format by default 
    
    lcsCandidates = sorted(lcsCandidates)
    ...
    if kargs.get('save_', True): 
        save_tset(Xs, Ls, meta=patternType)
...
def t_lcs(**kargs): 
    ...
    def analyze_the_other_strata(current_label, df_lcs, pattern_type='enriched', save_=True):  
        ...
        for label, entry in stratified.items(): 
            ...
            # query frequency: number of docs in the other strata in which lcs occurs
            # [note] candiate attributes: ['length', 'lcs', 'n_uniq', 'count', 'df', ]
            ret = analyzeMCS(Di, Ti, lcs_set=lcs_set, make_color_time=False, check_df_min=False)  # index:lcsmap, frequency:lcsmapInvFreq
    ...
    def save_relative_stats(df, label, meta='enriched'):  
        ...
        Pathway.save(df, cohort=cohort, scope=ltype, policy=lcs_select_policy, suffix=suffix, dir_type='pathway')
        return   
    def sample_lcs_set(df, max_n=None, pattern_type='enriched', criteria=['count', ]):
        ...
        if df.shape[0] < max_n: print('sample_lcs_set> n: %d but desired: %d (max_n)' % (df.shape[0], max_n))
    ...
    for label, entry in stratified.items():
        ...
        if df is None or df.shape[0] < nLCSLocal: 
            print('... precomputed df has %d < %d patterns > adding new ones ...' % (df.shape[0], nLCSLocal))
            # ['length', 'lcs', 'n_uniq', 'count', 'df', ] 
            df = deriveLCS(D=Di, 
                    # inputdir=inputdir, ifiles=ifiles,
                    
                    topn_lcs=kargs.get('topn_lcs', nLCSLocal), # "topN" for enriched and rare
                    min_length=kargs.get('min_length', minLength), max_length=kargs.get('max_length', maxLength), 
            ...
            df = sample_lcs_set(df, max_n=topNLCS, pattern_type=patternType, criteria=['count', ])
...
def runExperimentGloballyDerivedLCS(**kargs):  # global counterpart of runExperimentLocallyDerivedLCS() 
    ...
    # ct0 = lcsHandler.ctype # to restore to the original 
    ct = ctypes = ['diag', 'med', ] # ['regular', 'diag', 'med', ]
    pt = lcsHandler.pattern_types # ['enriched', 'rare']
    settings = [ct, pt, ]
    for setting in itertools.product(*settings):
        ctype, pattern_type = setting  # [hardcode]
...
def t_lcs2(**kargs):
    ...
    def save_relative_stats(df, label, meta='enriched'):  
        ...
        return
    def sample_lcs_set(df, max_n=None, pattern_type='enriched', criteria=['count', ]):
        # use: df = sample_lcs_set(df, max_n=topNLCS, pattern_type=patternType, criteria=['count', ])
...
def t_make_lcs_fset(**kargs):
    ...
    sq.sysConfig(cohort=cohort, seq_ptype=ctype, 
        lcs_type=lcsType, lcs_policy=lcsPolicy, 
        consolidate_lcs=True, 
        slice_policy=kargs.get('slice_policy', 'noop'), 
    ...
    if tCompleteData: 
        if tMakeTSet:
            kargs['topn'] = topn = 10000  # select a total of this many features
            kargs['pattern_type'] = 'rare'
...
def test(**kargs): 
    ...
    # runExperimentLocallyDerivedLCS(inputdir=inputdir)  # i.e. t_lcs_local(): derive local LCSs from within each stratum


    ### global -> stratified LCS patterns
    runExperimentGloballyDerivedLCS(inputdir=inputdir, load_lcs_stats=True)  
    ...
    # t_make_lcs_fset(lcs_var_policy='local', pattern_type='rare', topn=10000) # i.e. t_lcs2a() | default: topn=10K, pattern_type='rare'
